...
var Utils$1 = class Utils {
  ...
  static getLargestStringFromArr(arr) {
    return arr.reduce((a2, b2) => {
      if (Array.isArray(b2)) {
        b2 = b2.reduce((aa, bb) => aa.length > bb.length ? aa : bb);
      }
  ...
  shadeRGBColor(percent, color) {
    let f2 = color.split(","), t2 = percent < 0 ? 0 : 255, p2 = percent < 0 ? percent * -1 : percent, R = parseInt(f2[0].slice(4), 10), G = parseInt(f2[1], 10), B = parseInt(f2[2], 10);
    return "rgb(" + (Math.round((t2 - R) * p2) + R) + "," + (Math.round((t2 - G) * p2) + G) + "," + (Math.round((t2 - B) * p2) + B) + ")";
  }
  shadeHexColor(percent, color) {
    let f2 = parseInt(color.slice(1), 16), t2 = percent < 0 ? 0 : 255, p2 = percent < 0 ? percent * -1 : percent, R = f2 >> 16, G = f2 >> 8 & 255, B = f2 & 255;
    return "#" + (16777216 + (Math.round((t2 - R) * p2) + R) * 65536 + (Math.round((t2 - G) * p2) + G) * 256 + (Math.round((t2 - B) * p2) + B)).toString(16).slice(1);
  }
  // beautiful color shading blending code
  // http://stackoverflow.com/questions/5560248/programmatically-lighten-or-darken-a-hex-color-or-rgb-and-blend-colors
  shadeColor(p2, color) {
    if (Utils.isColorHex(color)) {
      return this.shadeHexColor(p2, color);
    } else {
      return this.shadeRGBColor(p2, color);
    }
  }
  ...
  static preciseAddition(a2, b2) {
    let aDecimals = (String(a2).split(".")[1] || "").length;
    let bDecimals = (String(b2).split(".")[1] || "").length;
    let factor = Math.pow(10, Math.max(aDecimals, bDecimals));
    return (Math.round(a2 * factor) + Math.round(b2 * factor)) / factor;
  }
  ...
  //
  static getGCD(a2, b2, p2 = 7) {
    let factor = Math.pow(10, p2 - Math.floor(Math.log10(Math.max(a2, b2))));
    if (factor > 1) {
      a2 = Math.round(Math.abs(a2) * factor);
      b2 = Math.round(Math.abs(b2) * factor);
    } else {
      factor = 1;
    }
    while (b2) {
  ...
  static mod(a2, b2, p2 = 7) {
    let big = Math.pow(10, p2 - Math.floor(Math.log10(Math.max(a2, b2))));
    a2 = Math.round(Math.abs(a2) * big);
    b2 = Math.round(Math.abs(b2) * big);
    return a2 % b2 / big;
  }
};
...
var Graphics = class _Graphics {
  ...
  drawCircle(radius, attrs = null) {
    ...
    const c2 = w.globals.dom.Paper.circle(radius * 2);
    if (attrs !== null) {
      c2.attr(attrs);
    }
    return c2;
  ...
  drawPath({
    ...
    let p2 = w.globals.dom.Paper.path(d).attr({
      fill,
  ...
  curve(x1, y1, x2, y2, x, y3) {
    let curve = ["C", x1, y1, x2, y2, x, y3].join(" ");
    return curve;
  }
  quadraticCurve(x1, y1, x, y2) {
    let curve = ["Q", x1, y1, x, y2].join(" ");
    return curve;
  }
  arc(rx, ry, axisRotation, largeArcFlag, sweepFlag, x, y2, relative = false) {
    let coord = "A";
    if (relative) coord = "a";
    let arc = [coord, rx, ry, axisRotation, largeArcFlag, sweepFlag, x, y2].join(
      " "
    );
    return arc;
  }
  ...
  drawPattern(style, width, height, stroke = "#a8a8a8", strokeWidth = 0) {
    let w = this.w;
    let p2 = w.globals.dom.Paper.pattern(width, height, (add) => {
  ...
  drawGradient(style, gfrom, gto, opacityFrom, opacityTo, size = null, stops = null, colorStops = [], i2 = 0) {
    let w = this.w;
    let g;
    ...
    if (!colorStops || colorStops.length === 0) {
      ...
      g = w.globals.dom.Paper.gradient(radial ? "radial" : "linear", (add) => {
        let gradientStops = Array.isArray(colorStops[i2]) ? colorStops[i2] : colorStops;
  ...
  getMarkerPath(x, y2, type, size) {
    let d = "";
    switch (type) {
      case "cross":
        size = size / 1.4;
        d = `M ${x - size} ${y2 - size} L ${x + size} ${y2 + size}  M ${x - size} ${y2 + size} L ${x + size} ${y2 - size}`;
        break;
      case "plus":
        size = size / 1.12;
        d = `M ${x - size} ${y2} L ${x + size} ${y2}  M ${x} ${y2 - size} L ${x} ${y2 + size}`;
        break;
      case "star":
      case "sparkle": {
        let points = 5;
        ...
        for (let i2 = 0; i2 <= 2 * points; i2++) {
          ...
          const yPos = y2 - radius * Math.cos(angle);
   ...
   */
  drawMarkerShape(x, y2, type, size, opts) {
    const path = this.drawPath({
      d: this.getMarkerPath(x, y2, type, size, opts),
  ...
  drawMarker(x, y2, opts) {
    x = x || 0;
    let size = opts.pSize || 0;
    if (!Utils$1.isNumber(y2)) {
      size = 0;
      y2 = 0;
    }
    return this.drawMarkerShape(x, y2, opts == null ? void 0 : opts.shape, size, __spreadValues(__spreadValues({}, opts), opts.shape === "line" || opts.shape === "plus" || opts.shape === "cross" ? {
  ...
  pathMouseEnter(path, e2) {
    let w = this.w;
    const filters = new Filters(this.ctx);
    const i2 = parseInt(path.node.getAttribute("index"), 10);
    ...
    if (w.config.states.hover.filter.type !== "none") {
      if (!w.globals.isTouchDevice) {
        ...
        filters.applyFilter(path, i2, hoverFilter.type);
      }
    }
  }
  ...
  pathMouseDown(path, e2) {
    ...
    if (path.node.getAttribute("selected") === "true") {
      ...
      if (w.globals.selectedDataPoints[i2].indexOf(j) > -1) {
        let index = w.globals.selectedDataPoints[i2].indexOf(j);
        w.globals.selectedDataPoints[i2].splice(index, 1);
      }
    } else {
      ...
      selected = "true";
      if (typeof w.globals.selectedDataPoints[i2] === "undefined") {
        w.globals.selectedDataPoints[i2] = [];
      }
      w.globals.selectedDataPoints[i2].push(j);
    }
    if (selected === "true") {
      let activeFilter = w.config.states.active.filter;
      if (activeFilter !== "none") {
        filters.applyFilter(path, i2, activeFilter.type);
      } else {
  ...
  setupEventDelegation(parentGroup, targetSelector) {
    ...
    parentGroup.node.addEventListener("mouseover", (e2) => {
      ...
      if (currentHovered && currentHovered.instance) {
        this.pathMouseLeave(currentHovered.instance, e2);
      }
    ...
    parentGroup.node.addEventListener("mouseout", (e2) => {
      ...
      const relatedNode = e2.relatedTarget ? _Graphics._findDelegateTarget(
        e2.relatedTarget,
        parentGroup.node,
        targetSelector
      ) : null;
...
var CoreUtils = class {
  ...
  static checkComboSeries(series, chartType) {
    ...
    if (series.length && typeof series[0].type !== "undefined") {
      series.forEach((s2) => {
        if (s2.type === "bar" || s2.type === "column" || s2.type === "candlestick" || s2.type === "boxPlot") {
          comboBarCount++;
        }
  ...
  getStackedSeriesTotals(excludedSeriesIndices = []) {
    ...
    for (let i2 = 0; i2 < w.globals.series[w.globals.maxValsInArrayIndex].length; i2++) {
      let t2 = 0;
      for (let j = 0; j < w.globals.series.length; j++) {
        if (typeof w.globals.series[j][i2] !== "undefined" && excludedSeriesIndices.indexOf(j) === -1) {
          t2 += w.globals.series[j][i2];
        }
      }
      total.push(t2);
    }
  ...
  getStackedSeriesTotalsByGroups() {
    ...
    w.globals.seriesGroups.forEach((sg) => {
      ...
      const excludedIndices = w.globals.series.map((_, fi) => includedIndexes.indexOf(fi) === -1 ? fi : -1).filter((f2) => f2 !== -1);
  ...
  setSeriesYAxisMappings() {
    ...
    let seriesNameArrayStyle = gl.series.length > cnf.yaxis.length || cnf.yaxis.some((a2) => Array.isArray(a2.seriesName));
            ...
            if (s2.name === name2) {
              ...
              if (yi === si || seriesNameArrayStyle) {
                if (!seriesNameArrayStyle || unassignedSeriesIndices.indexOf(si) > -1) {
                  ...
                  console.warn(
                    "Series '" + s2.name + "' referenced more than once in what looks like the new style. That is, when using either seriesName: [], or when there are more series than yaxes."
                  );
                }
              } else {
  ...
  isSeriesNull(index = null) {
    let r2 = [];
    if (index === null) {
      r2 = this.w.config.series.filter((d) => d !== null);
    } else {
      r2 = this.w.config.series[index].data.filter((d) => d !== null);
    }
    return r2.length === 0;
  }
  seriesHaveSameValues(index) {
    return this.w.globals.series[index].every((val, i2, arr) => val === arr[0]);
  }
  ...
  getLargestSeries() {
    const w = this.w;
    w.globals.maxValsInArrayIndex = w.globals.series.map((a2) => a2.length).indexOf(
      Math.max.apply(
        Math,
        w.globals.series.map((a2) => a2.length)
      )
    );
  }
  ...
  getCalculatedRatios() {
    ...
    for (let i2 = 0; i2 < gl.yRange.length; i2++) {
      yRatio.push(gl.yRange[i2] / gl.gridHeight);
    }
    ...
    if (w.globals.seriesYAxisReverseMap.length > 0) {
      let scaleBaseLineYScale = (y2, i2) => {
        let yAxis = w.config.yaxis[w.globals.seriesYAxisReverseMap[i2]];
        let sign = y2 < 0 ? -1 : 1;
        y2 = Math.abs(y2);
        if (yAxis.logarithmic) {
          y2 = this.getBaseLog(yAxis.logBase, y2);
        }
        return -sign * y2 / yRatio[i2];
      };
      if (gl.isMultipleYAxis) {
        baseLineY = [];
  ...
  getLogSeries(series) {
    ...
    w.globals.seriesLog = series.map((s2, i2) => {
      let yAxisIndex = w.globals.seriesYAxisReverseMap[i2];
  ...
  getLogVal(b2, d, seriesIndex) {
    if (d <= 0) {
      return 0;
    }
    const w = this.w;
    const min_log_val = w.globals.minYArr[seriesIndex] === 0 ? -1 : this.getBaseLog(b2, w.globals.minYArr[seriesIndex]);
    const max_log_val = w.globals.maxYArr[seriesIndex] === 0 ? 0 : this.getBaseLog(b2, w.globals.maxYArr[seriesIndex]);
    const number_of_height_levels = max_log_val - min_log_val;
    if (d < 1) return d / number_of_height_levels;
    const log_height_value = this.getBaseLog(b2, d) - min_log_val;
  ...
  getLogYRatios(yRatio) {
    ...
    gl.logYRange = gl.yRange.map((_, i2) => {
      ...
      if (w.config.yaxis[yAxisIndex] && this.w.config.yaxis[yAxisIndex].logarithmic) {
        ...
        gl.yLogRatio[i2] = range / gl.gridHeight;
        return range;
      }
    });
    ...
    if (typeSeries.series.length > 0) {
      typeGroups.forEach((gn) => {
        ...
        typeSeries.i.forEach((i2, ii) => {
          if (w.config.series[i2].group === gn) {
...
var Helpers$4 = class Helpers {
  ...
  setOrientations(anno, annoIndex = null) {
    ...
    if (anno.label.orientation === "vertical") {
      ...
      if (xAnno !== null) {
        ...
        const { x, y: y2 } = this.annoCtx.graphics.rotateAroundCenter(xAnno);
        xAnno.setAttribute("transform", `rotate(-90 ${x} ${y2})`);
      }
    }
  }
  ...
  annotationsBackground() {
    ...
    w.config.annotations.xaxis.forEach((anno, i2) => add(anno, i2, "xaxis"));
    w.config.annotations.yaxis.forEach((anno, i2) => add(anno, i2, "yaxis"));
    w.config.annotations.points.forEach((anno, i2) => add(anno, i2, "point"));
  }
  getY1Y2(type, anno) {
    var _a;
    const w = this.w;
    let y2 = type === "y1" ? anno.y : anno.y2;
    let yP;
    let clipped = false;
    if (this.annoCtx.invertAxis) {
      const labels = w.config.xaxis.convertedCatToNumeric ? w.globals.categoryLabels : w.globals.labels;
      const catIndex = labels.indexOf(y2);
      ...
      const yPos = w.config.yaxis[anno.yAxisIndex].logarithmic ? new CoreUtils(this.annoCtx.ctx).getLogVal(
        ...
        y2,
        seriesIndex
      ) / w.globals.yLogRatio[seriesIndex] : (y2 - w.globals.minYArr[seriesIndex]) / (w.globals.yRange[seriesIndex] / w.globals.gridHeight);
    ...
    if (typeof y2 === "string" && y2.includes("px")) {
      yP = parseFloat(y2);
    }
    return { yP, clipped };
  }
...
var DateTime = class {
  ...
  formatDate(date, format) {
    ...
    function ii(i2, len) {
      let s22 = i2 + "";
      len = len || 2;
      while (s22.length < len) s22 = "0" + s22;
      return s22;
    }
    let y2 = utc ? date.getUTCFullYear() : date.getFullYear();
    format = format.replace(/(^|[^\\])yyyy+/g, "$1" + y2);
    format = format.replace(/(^|[^\\])yy/g, "$1" + y2.toString().substr(2, 2));
    format = format.replace(/(^|[^\\])y/g, "$1" + y2);
    ...
    format = format.replace(/(^|[^\\])mm+/g, "$1" + ii(m));
    format = format.replace(/(^|[^\\])m/g, "$1" + m);
    let s2 = utc ? date.getUTCSeconds() : date.getSeconds();
    format = format.replace(/(^|[^\\])ss+/g, "$1" + ii(s2));
    format = format.replace(/(^|[^\\])s/g, "$1" + s2);
    let f2 = utc ? date.getUTCMilliseconds() : date.getMilliseconds();
    format = format.replace(/(^|[^\\])fff+/g, "$1" + ii(f2, 3));
    f2 = Math.round(f2 / 10);
    format = format.replace(/(^|[^\\])ff/g, "$1" + ii(f2));
    f2 = Math.round(f2 / 10);
    format = format.replace(/(^|[^\\])f/g, "$1" + f2);
    let T = H < 12 ? "AM" : "PM";
    format = format.replace(/(^|[^\\])TT+/g, "$1" + T);
    format = format.replace(/(^|[^\\])T/g, "$1" + T.charAt(0));
    let t2 = T.toLowerCase();
  ...
  defaultYFormatter(v, yaxe) {
    ...
    if (Utils$1.isNumber(v)) {
      if (w.globals.yValueDecimal !== 0) {
        ...
        const f2 = v.toFixed(0);
        v = v == f2 ? f2 : v.toFixed(1);
      }
    }
    return v;
  }
  setLabelFormatters() {
    ...
    w.config.yaxis.forEach((yaxe, i2) => {
      if (yaxe.labels.formatter !== void 0) {
        w.globals.yLabelFormatters[i2] = yaxe.labels.formatter;
      } else {
        w.globals.yLabelFormatters[i2] = (val) => {
          if (!w.globals.xyCharts) return val;
          if (Array.isArray(val)) {
            return val.map((v) => {
              return this.defaultYFormatter(v, yaxe, i2);
            });
          } else {
            return this.defaultYFormatter(val, yaxe, i2);
          }
        };
      }
    });
  ...
  heatmapLabelFormatters() {
    ...
    if (w.config.chart.type === "heatmap") {
      ...
      let longest = w.globals.seriesNames.reduce(
        (a2, b2) => a2.length > b2.length ? a2 : b2,
        0
      );
...
var AxesUtils = class {
  ...
  getLabel(labels, timescaleLabels, x, i2, drawnLabels = [], fontSize = "12px", isLeafGroup = true) {
    const w = this.w;
    let rawLabel = typeof labels[i2] === "undefined" ? "" : labels[i2];
    let label = rawLabel;
    ...
    const determineHighestUnit = (unit) => {
      ...
      timescaleLabels.forEach((t2) => {
        if (t2.unit === "month") {
          highestUnit = "year";
        } else if (t2.unit === "day") {
          highestUnit = "month";
        } else if (t2.unit === "hour") {
          highestUnit = "day";
        } else if (t2.unit === "minute") {
          highestUnit = "hour";
        }
      });
    ...
    if (timescaleLabels.length > 0) {
      isBold = determineHighestUnit(timescaleLabels[i2].unit);
  ...
  translateYAxisIndex(index) {
    ...
    let newStyle = gl.series.length > yaxis.length || yaxis.some((a2) => Array.isArray(a2.seriesName));
    if (newStyle) {
      return index;
    } else {
  ...
  addPointAnnotation(anno, parent, index) {
    ...
    if (!(clipY || clipX)) {
      ...
      let elText = this.annoCtx.graphics.drawText({
        ...
        y: y2 + anno.label.offsetY - anno.marker.size - parseFloat(anno.label.style.fontSize) / 1.6,
        text,
        textAnchor: anno.label.textAnchor,
        fontSize: anno.label.style.fontSize,
        fontFamily: anno.label.style.fontFamily,
      ...
      if (anno.customSVG.SVG) {
        ...
        g.attr({
          transform: `translate(${x + anno.customSVG.offsetX}, ${y2 + anno.customSVG.offsetY})`
        });
      ...
      if (anno.image.path) {
        ...
        point = this.annoCtx.addImage({
          ...
          y: y2 + anno.image.offsetY - imgHeight / 2,
          width: imgWidth,
          height: imgHeight,
        ...
        radialBar: {
          ...
          dataLabels: {
            ...
            total: {
              ...
              color: void 0,
              formatter(w) {
                return w.globals.seriesTotals.reduce((a2, b2) => a2 + b2, 0) / w.globals.series.length + "%";
              }
            }
          },
          barLabels: {
          ...
          donut: {
            ...
            labels: {
              ...
              total: {
                ...
                color: void 0,
                formatter(w) {
                  return w.globals.seriesTotals.reduce((a2, b2) => a2 + b2, 0);
                }
              }
            }
          }
...
var Annotations = class {
  ...
  drawAxesAnnotations() {
    ...
    if (w.globals.axisCharts && w.globals.dataPoints) {
      ...
      for (let i2 = 0; i2 < 3; i2++) {
        w.globals.dom.elGraphical.add(annoArray[i2]);
        if (initialAnim && !w.globals.resized && !w.globals.dataChanged) {
  ...
  drawImageAnnos() {
    const w = this.w;
    w.config.annotations.images.map((s2, index) => {
      this.addImage(s2, index);
    });
  }
  drawTextAnnos() {
    const w = this.w;
    w.config.annotations.texts.map((t2, index) => {
      this.addText(t2, index);
    });
  }
  addXaxisAnnotation(anno, parent, index) {
  ...
  addImage(params) {
    ...
    img.size(width, height).move(x, y2);
    const parent = w.globals.dom.baseEl.querySelector(appendTo);
    if (parent) {
...
var Defaults = class {
  ...
  _getBoxTooltip(w, seriesIndex, dataPointIndex, labels, chartType) {
    const o2 = w.globals.seriesCandleO[seriesIndex][dataPointIndex];
    const h2 = w.globals.seriesCandleH[seriesIndex][dataPointIndex];
    const m = w.globals.seriesCandleM[seriesIndex][dataPointIndex];
    const l2 = w.globals.seriesCandleL[seriesIndex][dataPointIndex];
    const c2 = w.globals.seriesCandleC[seriesIndex][dataPointIndex];
...
var Markers = class {
  ...
  setGlobalMarkerSize() {
    ...
    if (w.globals.markers.size.length > 0) {
      if (w.globals.markers.size.length < w.globals.series.length + 1) {
        for (let i2 = 0; i2 <= w.globals.series.length; i2++) {
  ...
  plotChartMarkers({
    ...
    if (Array.isArray(p2.x)) {
      for (let q = 0; q < p2.x.length; q++) {
        let markerElement;
        let dataPointIndex = j;
        let invalidMarker = !Utils$1.isNumber(p2.y[q]);
        if (w.globals.markers.largestSize === 0 && w.globals.hasNullValues && w.globals.series[i2][j + 1] !== null && !isVirtualPoint) {
          invalidMarker = true;
        }
        ...
        if (shouldMarkerDraw || alwaysDrawMarker || hasDiscreteMarkers) {
          ...
          if (w.config.series[i2].data[dataPointIndex]) {
            if (w.config.series[i2].data[dataPointIndex].fillColor) {
              opts.pointFillColor = w.config.series[i2].data[dataPointIndex].fillColor;
            }
            if (w.config.series[i2].data[dataPointIndex].strokeColor) {
              opts.pointStrokeColor = w.config.series[i2].data[dataPointIndex].strokeColor;
            }
          }
          if (typeof pSize !== "undefined") {
            opts.pSize = pSize;
          }
          if (p2.x[q] < -w.globals.markers.largestSize || p2.x[q] > w.globals.gridWidth + w.globals.markers.largestSize || p2.y[q] < -w.globals.markers.largestSize || p2.y[q] > w.globals.gridHeight + w.globals.markers.largestSize) {
            opts.pSize = 0;
          }
          if (!invalidMarker) {
            ...
            markerElement = graphics.drawMarker(p2.x[q], p2.y[q], opts);
          ...
          w.globals.pointsArray[seriesIndex].push([p2.x[q], p2.y[q]]);
        }
      }
    }
...
var Scatter = class {
  ...
  draw(elSeries, j, opts) {
    ...
    if (Array.isArray(pointsPos.x)) {
      for (let q = 0; q < pointsPos.x.length; q++) {
        ...
        let x = pointsPos.x[q];
        let y2 = pointsPos.y[q];
        radius = radius || 0;
        if (y2 === null || typeof w.globals.series[realIndex][dataPointIndex] === "undefined") {
          shouldDraw = false;
        }
        if (shouldDraw) {
  ...
  drawPoint(x, y2, radius, realIndex, dataPointIndex, j) {
    const w = this.w;
    let i2 = realIndex;
    ...
    let el = graphics.drawMarker(x, y2, markerConfig);
    if (w.config.series[i2].data[dataPointIndex]) {
      if (w.config.series[i2].data[dataPointIndex].fillColor) {
        pathFillCircle = w.config.series[i2].data[dataPointIndex].fillColor;
      }
    }
    el.attr({
      fill: pathFillCircle
  ...
  centerTextInBubble(y2) {
    let w = this.w;
    y2 = y2 + parseInt(w.config.dataLabels.style.fontSize, 10) / 4;
    return {
      y: y2
    };
  }
...
var DataLabels = class {
  ...
  dataLabelsCorrection(x, y2, val, i2, dataPointIndex, alwaysDrawDataLabel, fontSize) {
    let w = this.w;
    let graphics = new Graphics(this.ctx);
    ...
    if (y2 < 0) y2 = 0;
    if (y2 > w.globals.gridHeight + height) y2 = w.globals.gridHeight + height / 2;
    if (typeof w.globals.dataLabelsRects[i2] === "undefined")
      w.globals.dataLabelsRects[i2] = [];
    w.globals.dataLabelsRects[i2].push({ x, y: y2, width, height });
    let len = w.globals.dataLabelsRects[i2].length - 2;
    let lastDrawnIndex = typeof w.globals.lastDrawnDataLabelsIndexes[i2] !== "undefined" ? w.globals.lastDrawnDataLabelsIndexes[i2][w.globals.lastDrawnDataLabelsIndexes[i2].length - 1] : 0;
    if (typeof w.globals.dataLabelsRects[i2][len] !== "undefined") {
      let lastDataLabelRect = w.globals.dataLabelsRects[i2][lastDrawnIndex];
      if (
        // next label forward and x not intersecting
        x > lastDataLabelRect.x + lastDataLabelRect.width || y2 > lastDataLabelRect.y + lastDataLabelRect.height || y2 + height < lastDataLabelRect.y || x + width < lastDataLabelRect.x
      ) {
        drawnextLabel = true;
      }
    }
  ...
  drawDataLabel({ type, pos, i: i2, j, isRangeStart, strokeWidth = 2 }) {
    ...
    for (let q = 0; q < pos.x.length; q++) {
      ...
      y2 = pos.y[q] + dataLabelsConfig.offsetY + strokeWidth;
      if (!isNaN(x)) {
        if (j === 1 && q === 0) dataPointIndex = 0;
        if (j === 1 && q === 1) dataPointIndex = 1;
        let val = w.globals.series[i2][dataPointIndex];
        if (type === "rangeArea") {
          if (isRangeStart) {
            val = w.globals.seriesRangeStart[i2][dataPointIndex];
          } else {
            val = w.globals.seriesRangeEnd[i2][dataPointIndex];
          }
        }
        let text = "";
        const getText = (v) => {
        ...
        if (w.config.chart.type === "bubble") {
          val = w.globals.seriesZ[i2][dataPointIndex];
          text = getText(val);
          y2 = pos.y[q];
        ...
        if (w.globals.isSlopeChart) {
          if (dataPointIndex === 0) {
            textAnchor = "end";
          } else if (dataPointIndex === w.config.series[i2].data.length - 1) {
            textAnchor = "start";
          } else {
            textAnchor = "middle";
          }
  ...
  plotDataLabelsText(opts) {
    ...
    if (w.globals.isSlopeChart) {
      ...
      if (j !== 0 && j !== w.config.series[i2].data.length - 1) {
        offX = 0;
      }
    }
...
var Series = class {
  ...
  highlightRangeInSeries(e2, targetElement) {
    const w = this.w;
    ...
    const activeInactive = (action) => {
      for (let i2 = 0; i2 < allHeatMapElements.length; i2++) {
        allHeatMapElements[i2].classList[action](this.legendInactiveClass);
      }
    };
  ...
  getActiveConfigSeriesIndex(order = "asc", chartTypes = []) {
    ...
    if (w.config.series.length > 1) {
      let activeSeriesIndex = w.config.series.map((s2, index) => {
        ...
        const hasData = s2.data && s2.data.length > 0 && w.globals.collapsedSeriesIndices.indexOf(index) === -1;
  ...
  getPreviousPaths() {
    ...
    function pushPaths(seriesEls, i2, type) {
      let paths = seriesEls[i2].childNodes;
      let dArr = {
        type,
        paths: [],
    ...
    if (heatTreeSeries.length > 0) {
      for (let h2 = 0; h2 < heatTreeSeries.length; h2++) {
        ...
        for (let i2 = 0; i2 < seriesEls.length; i2++) {
          const getAttr = (x) => {
            return seriesEls[i2].getAttribute(x);
          };
          const rect = {
  ...
  handleNoData() {
    ...
    if (noDataOpts.verticalAlign === "top") {
      ...
      y2 = w.globals.svgHeight - 50;
    }
    x = x + noDataOpts.offsetX;
    y2 = y2 + parseInt(noDataOpts.style.fontSize, 10) + 2 + noDataOpts.offsetY;
...
var Data = class {
  ...
  handleFormat2DArray(ser, i2) {
    ...
    const isBoxPlot = cnf.chart.type === "boxPlot" || cnf.series[i2].type === "boxPlot";
    for (let j = 0; j < data.length; j++) {
      ...
      if (typeof y2 !== "undefined") {
        if (Array.isArray(y2) && y2.length === 4 && !isBoxPlot) {
          this.twoDSeries.push(Utils$1.parseNumber(y2[3]));
        } else if (point.length >= 5) {
          this.twoDSeries.push(Utils$1.parseNumber(point[4]));
        } else {
          this.twoDSeries.push(Utils$1.parseNumber(y2));
        }
  ...
  handleRangeData(ser, i2) {
    ...
    gl.seriesRangeStart[i2] = range.start === void 0 ? [] : range.start;
    gl.seriesRangeEnd[i2] = range.end === void 0 ? [] : range.end;
    gl.seriesRange[i2] = range.rangeUniques;
  ...
  handleCandleStickBoxData(ser, i2) {
    ...
    gl.seriesCandleO[i2] = ohlc.o;
    gl.seriesCandleH[i2] = ohlc.h;
    gl.seriesCandleM[i2] = ohlc.m;
    gl.seriesCandleL[i2] = ohlc.l;
    gl.seriesCandleC[i2] = ohlc.c;
    return ohlc;
  }
  handleRangeDataFormat(format, ser, i2) {
    ...
    if (format === "array") {
      ...
      for (let j = 0; j < ser[i2].data.length; j++) {
        let isDataPoint2D = Array.isArray(ser[i2].data[j].y);
        ...
        ser[i2].data[j].rangeName = id;
        const keyObj = uniqueKeysMap.get(x);
        if (keyObj) {
          keyObj.y.push(y2);
        }
        rangeStart.push(y2.y1);
        rangeEnd.push(y2.y2);
      }
    }
    return {
  ...
  handleCandleStickBoxDataFormat(format, ser, i2) {
    const w = this.w;
    const isBoxPlot = w.config.chart.type === "boxPlot" || w.config.series[i2].type === "boxPlot";
    const serO = [];
    const serH = [];
    const serM = [];
    const serL = [];
  ...
  parseDataAxisCharts(ser, ctx = this.ctx) {
    ...
    ser.forEach((s2, i2) => {
      if (s2.name !== void 0) {
        gl.seriesNames.push(s2.name);
      } else {
        gl.seriesNames.push("series-" + parseInt(i2 + 1, 10));
      }
    });
    this.coreUtils.setSeriesYAxisMappings();
    let buckets = [];
    let groups = [...new Set(cnf.series.map((s2) => s2.group))];
    cnf.series.forEach((s2, i2) => {
      let index = groups.indexOf(s2.group);
      if (!buckets[index]) buckets[index] = [];
      buckets[index].push(gl.seriesNames[i2]);
    });
    ...
    for (let i2 = 0; i2 < ser.length; i2++) {
      ...
      if (this.isMultiFormat()) {
        ...
        gl.labels.push(this.twoDSeriesX);
        const singleArray = ser[i2].data.map((d) => Utils$1.parseNumber(d));
        gl.series.push(singleArray);
      }
      gl.seriesZ.push(this.threeDSeries);
      if (ser[i2].color !== void 0) {
        gl.seriesColors.push(ser[i2].color);
      } else {
  ...
  parseDataNonAxisCharts(ser) {
    ...
    if (hasOldFormat) {
      ...
      for (let i2 = 0; i2 < gl.series.length; i2++) {
        if (gl.seriesNames[i2] === void 0) {
          gl.seriesNames.push("series-" + (i2 + 1));
        }
      }
      return this.w;
    }
    if (Array.isArray(ser) && ser.every((s2) => typeof s2 === "number")) {
      ...
      gl.seriesNames = [];
      for (let i2 = 0; i2 < gl.series.length; i2++) {
        gl.seriesNames.push(cnf.labels[i2] || `series-${i2 + 1}`);
      }
      return this.w;
    }
  ...
  _generateExternalLabels(ser) {
        ...
        if (this.isFormatXY()) {
          const seriesDataFiltered = cnf.series.map((serie) => {
            return serie.data.filter(
              (v, i2, a2) => a2.findIndex((t2) => t2.x === v.x) === i2
            );
          });
          const len = seriesDataFiltered.reduce(
            (p2, c2, i2, a2) => a2[p2].length > c2.length ? p2 : i2,
            0
          );
          ...
          for (let i2 = 0; i2 < gl.series[gl.maxValsInArrayIndex].length; i2++) {
            labelArr.push(i2 + 1);
          }
        }
      }
    ...
    gl.labels = labelArr;
    if (cnf.xaxis.convertedCatToNumeric) {
      gl.categoryLabels = labelArr.map((l2) => {
  ...
  parseRawDataIfNeeded(series) {
    ...
    if (!globalParsing && !series.some((s2) => s2.parsing)) {
      return series;
    }
    const processedSeries = series.map((serie, index) => {
      ...
      const transformedData = serie.data.map((item, itemIndex) => {
        ...
        if (Array.isArray(effectiveParsing.y)) {
          ...
          y2 = this.getNestedValue(item, effectiveParsing.y);
        }
...
var Exports = class {
  ...
  exportToCSV({
    ...
    let result = "";
    let universalBOM = "\uFEFF";
    let gSeries = w.globals.series.map((s2, i2) => {
      ...
      if (!w.globals.axisCharts) {
        ...
        if (w.config.xaxis.type === "category" || w.config.xaxis.convertedCatToNumeric) {
          if (w.globals.isBarHorizontal) {
            ...
            cat = lbFormatter(w.globals.labels[i2], {
    ...
    const handleAxisRowsColumns = (s2, sI) => {
      ...
      if (s2.data) {
        s2.data = s2.data.length && s2.data || getEmptyDataForCsvColumn();
        for (let i2 = 0; i2 < s2.data.length; i2++) {
          columns = [];
          let cat = getCat(i2);
          ...
          if (w.config.chart.type === "candlestick" || s2.type && s2.type === "candlestick") {
            columns.pop();
            columns.push(w.globals.seriesCandleO[sI][i2]);
            columns.push(w.globals.seriesCandleH[sI][i2]);
            columns.push(w.globals.seriesCandleL[sI][i2]);
            columns.push(w.globals.seriesCandleC[sI][i2]);
          }
          if (w.config.chart.type === "boxPlot" || s2.type && s2.type === "boxPlot") {
            columns.pop();
            columns.push(w.globals.seriesCandleO[sI][i2]);
            columns.push(w.globals.seriesCandleH[sI][i2]);
            columns.push(w.globals.seriesCandleM[sI][i2]);
            columns.push(w.globals.seriesCandleL[sI][i2]);
            columns.push(w.globals.seriesCandleC[sI][i2]);
          }
          if (w.config.chart.type === "rangeBar") {
            columns.pop();
            columns.push(w.globals.seriesRangeStart[sI][i2]);
            columns.push(w.globals.seriesRangeEnd[sI][i2]);
          }
          if (columns.length) {
            rows.push(columns.join(columnDelimiter));
          }
...
var Grid = class {
  ...
  _drawGridLine({ i: i2, x1, y1, x2, y2, xCount, parent }) {
    ...
    const offX = w.globals.barPadForNumericAxis;
    const excludeBorders = y1 === 0 && y2 === 0 || x1 === 0 && x2 === 0 || y1 === w.globals.gridHeight && y2 === w.globals.gridHeight || w.globals.isBarHorizontal && (i2 === 0 || i2 === xCount - 1);
  ...
  _drawXYLines({ xCount, tickAmount }) {
    ...
    const datetimeLines = ({ xC, x1, y1, x2, y2 }) => {
      for (let i2 = 0; i2 < xC; i2++) {
        x1 = this.xaxisLabels[i2].position;
        x2 = this.xaxisLabels[i2].position;
        this._drawGridLines({
          i: i2,
          x1,
          y1,
    ...
    const categoryLines = ({ xC, x1, y1, x2, y2 }) => {
      for (let i2 = 0; i2 < xC + (w.globals.isXNumeric ? 0 : 1); i2++) {
  ...
  drawGridBands(xCount, tickAmount) {
    ...
    if (((_b = w.config.grid.column.colors) == null ? void 0 : _b.length) > 0) {
      ...
      for (let i2 = 0, c2 = 0; i2 < xCount; i2++, c2++) {
        ...
        if (w.config.xaxis.type === "datetime") {
          x1 = this.xaxisLabels[i2].position;
          x2 = (((_c = this.xaxisLabels[i2 + 1]) == null ? void 0 : _c.position) || w.globals.gridWidth) - this.xaxisLabels[i2].position;
        }
...
var Scales = class {
  ...
  setYScaleForIndex(index, minY, maxY) {
    const gl = this.w.globals;
    const cnf = this.w.config;
    let y2 = gl.isBarHorizontal ? cnf.xaxis : cnf.yaxis[index];
    if (typeof gl.yAxisScale[index] === "undefined") {
      gl.yAxisScale[index] = [];
    }
    let range = Math.abs(maxY - minY);
    if (y2.logarithmic && range <= 5) {
      gl.invalidLogScale = true;
    }
    if (y2.logarithmic && range > 5) {
      gl.allSeriesCollapsed = false;
      gl.yAxisScale[index] = y2.forceNiceScale ? this.logarithmicScaleNice(minY, maxY, y2.logBase) : this.logarithmicScale(minY, maxY, y2.logBase);
    } else {
      ...
      if (axisSeries.length > 0) {
        ...
        if (cnf.chart.stacked) {
          ...
          for (let i2 = 0; i2 < axisSeries.length; i2++) {
            if (!seriesType && cnf.series[axisSeries[i2]].type) {
              seriesType = cnf.series[axisSeries[i2]].type;
            }
            let si = axisSeries[i2];
...
var Range = class {
  ...
  getMinYMaxY(startingSeriesIndex, lowestY = Number.MAX_VALUE, highestY = -Number.MAX_VALUE, endingSeriesIndex = null) {
    ...
    for (let i2 = startingSeriesIndex; i2 < endingSeriesIndex; i2++) {
      ...
      const seriesType = cnf.series[i2].type;
      if (gl.categoryLabels.length) {
        gl.dataPoints = gl.categoryLabels.filter(
          (label) => typeof label !== "undefined"
        ).length;
      }
      if (gl.labels.length && cnf.xaxis.type !== "datetime" && gl.series.reduce((a2, c2) => a2 + c2.length, 0) !== 0) {
      ...
      if (autoScaleYaxis) {
        if (cnf.xaxis.min) {
          for (; firstXIndex < lastXIndex && gl.seriesX[i2][firstXIndex] < cnf.xaxis.min; firstXIndex++) {
          }
        }
        if (cnf.xaxis.max) {
          for (; lastXIndex > firstXIndex && gl.seriesX[i2][lastXIndex] > cnf.xaxis.max; lastXIndex--) {
          }
        }
      }
      for (let j = firstXIndex; j <= lastXIndex && j < gl.series[i2].length; j++) {
        let val = series[i2][j];
        if (val !== null && Utils$1.isNumber(val)) {
          if (typeof ((_b = seriesMax[i2]) == null ? void 0 : _b[j]) !== "undefined") {
            maxY = Math.max(maxY, seriesMax[i2][j]);
            lowestY = Math.min(lowestY, seriesMax[i2][j]);
          }
          if (typeof ((_c = seriesMin[i2]) == null ? void 0 : _c[j]) !== "undefined") {
          ...
          switch (seriesType) {
            case "candlestick":
              {
                if (typeof gl.seriesCandleC[i2][j] !== "undefined") {
                  maxY = Math.max(maxY, gl.seriesCandleH[i2][j]);
                  lowestY = Math.min(lowestY, gl.seriesCandleL[i2][j]);
                }
              }
              break;
            case "boxPlot":
              {
                if (typeof gl.seriesCandleC[i2][j] !== "undefined") {
                  maxY = Math.max(maxY, gl.seriesCandleC[i2][j]);
                  lowestY = Math.min(lowestY, gl.seriesCandleO[i2][j]);
                }
              }
              break;
          }
          if (seriesType && seriesType !== "candlestick" && seriesType !== "boxPlot" && seriesType !== "rangeArea" && seriesType !== "rangeBar") {
            maxY = Math.max(maxY, gl.series[i2][j]);
            lowestY = Math.min(lowestY, gl.series[i2][j]);
          }
          if (gl.seriesGoals[i2] && gl.seriesGoals[i2][j] && Array.isArray(gl.seriesGoals[i2][j])) {
            gl.seriesGoals[i2][j].forEach((g) => {
          ...
          if (minY > ((_d = seriesMin[i2]) == null ? void 0 : _d[j]) && ((_e = seriesMin[i2]) == null ? void 0 : _e[j]) < 0) {
            minY = seriesMin[i2][j];
          }
        } else {
  ...
  setYRange() {
    ...
    cnf.series.forEach((s2) => {
      let type = s2.type || cnf.chart.type;
      switch (type) {
        case "bar":
        case "column":
          gl.barGroups.push(s2.group);
          break;
        case "line":
          gl.lineGroups.push(s2.group);
          break;
        case "area":
          gl.areaGroups.push(s2.group);
          break;
      }
    });
    gl.barGroups = gl.barGroups.filter((v, i2, a2) => a2.indexOf(v) === i2);
  ...
  setXRange() {
    ...
    const getInitialMinXMaxX = () => {
      for (let i2 = 0; i2 < gl.series.length; i2++) {
        if (gl.labels[i2]) {
          for (let j = 0; j < gl.labels[i2].length; j++) {
            if (gl.labels[i2][j] !== null && Utils$1.isNumber(gl.labels[i2][j])) {
              gl.maxX = Math.max(gl.maxX, gl.labels[i2][j]);
              gl.initialMaxX = Math.max(gl.maxX, gl.labels[i2][j]);
              gl.minX = Math.min(gl.minX, gl.labels[i2][j]);
              gl.initialMinX = Math.min(gl.minX, gl.labels[i2][j]);
            }
          }
        }
      }
  ...
  setZRange() {
    ...
    for (let i2 = 0; i2 < gl.series.length; i2++) {
      if (typeof gl.seriesZ[i2] !== "undefined") {
        for (let j = 0; j < gl.seriesZ[i2].length; j++) {
          if (gl.seriesZ[i2][j] !== null && Utils$1.isNumber(gl.seriesZ[i2][j])) {
            gl.maxZ = Math.max(gl.maxZ, gl.seriesZ[i2][j]);
            gl.minZ = Math.min(gl.minZ, gl.seriesZ[i2][j]);
          }
        }
      }
    }
    ...
    if (gl.isXNumeric) {
      gl.seriesX.forEach((sX) => {
        if (sX.length) {
          ...
          seriesX.sort((a2, b2) => a2 - b2);
          seriesX.forEach((s2, j) => {
            if (j > 0) {
              let xDiff = s2 - seriesX[j - 1];
              if (xDiff > 0) {
    ...
    seriesGroups.forEach((group) => {
      ...
      indicesOfSeriesInGroup.forEach((i2) => {
        ...
        for (let j = 0; j < gl.series[gl.maxValsInArrayIndex].length; j++) {
          ...
          let stackSeries = this.w.config.chart.stacked && !gl.comboCharts || this.w.config.chart.stacked && gl.comboCharts && (!this.w.config.chart.stackOnlyBar || ((_b = (_a = this.w.config.series) == null ? void 0 : _a[i2]) == null ? void 0 : _b.type) === "bar" || ((_d = (_c = this.w.config.series) == null ? void 0 : _c[i2]) == null ? void 0 : _d.type) === "column");
          if (stackSeries) {
            if (gl.series[i2][j] !== null && Utils$1.isNumber(gl.series[i2][j])) {
              gl.series[i2][j] > 0 ? stackedPoss[group][j] += parseFloat(gl.series[i2][j]) + 1e-4 : stackedNegs[group][j] += parseFloat(gl.series[i2][j]);
            }
          }
        }
      });
...
var YAxis = class {
  ...
  drawYaxis(realIndex) {
    ...
    if (w.config.yaxis[realIndex].labels.show) {
      ...
      let firstLabel = null;
      for (let i2 = tickAmount; i2 >= 0; i2--) {
        let val = lbFormatter(labels[i2], i2, w);
  ...
  drawYaxisInversed(realIndex) {
    ...
    if (w.config.xaxis.labels.show) {
      for (let i2 = timescaleLabels.length ? 0 : tickAmount; timescaleLabels.length ? i2 < timescaleLabels.length : i2 >= 0; timescaleLabels.length ? i2++ : i2--) {
        let val = lbFormatter(labels[i2], i2, w);
        let x = w.globals.gridWidth + w.globals.padHorizontal - (l2 - labelsDivider + w.config.xaxis.labels.offsetX);
...
var Events = class {
  ...
  setupEventHandlers() {
    ...
    this.ctx.eventList.forEach((event) => {
      clickableArea.addEventListener(
        event,
        (e2) => {
          let capturedSeriesIndex = e2.target.getAttribute("i") === null && w.globals.capturedSeriesIndex !== -1 ? w.globals.capturedSeriesIndex : e2.target.getAttribute("i");
          let capturedDataPointIndex = e2.target.getAttribute("j") === null && w.globals.capturedDataPointIndex !== -1 ? w.globals.capturedDataPointIndex : e2.target.getAttribute("j");
          const opts = Object.assign({}, w, {
  ...
  documentEvent(e2) {
    ...
    w.globals.clientX = e2.type === "touchmove" ? e2.touches[0].clientX : e2.clientX;
    w.globals.clientY = e2.type === "touchmove" ? e2.touches[0].clientY : e2.clientY;
  }
};
var Localization = class {
  ...
  setCurrentLocaleValues(localeName) {
    ...
    const selectedLocale = locales.filter((c2) => c2.name === localeName)[0];
    if (selectedLocale) {
...
var Responsive = class {
  ...
  checkResponsiveConfig(opts) {
    ...
    res.sort(
      (a2, b2) => a2.breakpoint > b2.breakpoint ? 1 : b2.breakpoint > a2.breakpoint ? -1 : 0
    ).reverse();
...
var Theme = class {
  ...
  getColors(configColors) {
    ...
    if (Array.isArray(configColors) && configColors.length > 0 && typeof configColors[0] === "function") {
      ...
      return w.config.series.map((s2, i2) => {
        let c2 = configColors[i2] || configColors[0];
        return typeof c2 === "function" ? c2({
          value: w.globals.axisCharts ? w.globals.series[i2][0] || 0 : w.globals.series[i2],
          seriesIndex: i2,
          dataPointIndex: i2,
          w: this.w
        }) : c2;
  ...
  applyColorTypes(colorTypes, defaultColors) {
    ...
    colorTypes.forEach((c2) => {
      ...
      this.pushExtraColors(w.globals[c2].colors);
    });
  }
...
var TitleSubtitle = class {
  ...
  drawTitleSubtitle(type) {
    ...
    y2 = y2 + parseInt(tsConfig.style.fontSize, 10) + tsConfig.margin / 2;
...
var Dimensions = class {
  ...
  conditionalChecksForAxisCoords(xaxisLabelCoords, xtitleCoords, xaxisGroupLabelCoords) {
    ...
    let minYAxisWidth = 0;
    let maxYAxisWidth = 0;
    w.config.yaxis.forEach((y2) => {
      minYAxisWidth += y2.labels.minWidth;
      maxYAxisWidth += y2.labels.maxWidth;
    });
    ...
    if (w.globals.axisCharts || w.config.chart.type === "radialBar") {
      ...
      if (isHidden) {
        ...
        seriesToMakeVisible.forEach((r2) => {
          this.riseCollapsedSeries(r2.cs, r2.csi, realIndex);
        });
      } else {
      ...
      for (let c2 = 0; c2 < collapsedSeries.length; c2++) {
        if (collapsedSeries[c2].index === realIndex) {
          if (w.globals.axisCharts) {
            ...
            series[realIndex] = collapsedSeries[c2].data;
          }
...
var Legend = class {
  ...
  createLegendMarker({ i: i2, fillcolor }) {
    ...
    let mSize = Array.isArray(w.config.legend.markers.size) ? parseFloat(w.config.legend.markers.size[i2]) : parseFloat(w.config.legend.markers.size);
    let mOffsetX = Array.isArray(w.config.legend.markers.offsetX) ? parseFloat(w.config.legend.markers.offsetX[i2]) : parseFloat(w.config.legend.markers.offsetX);
    let mOffsetY = Array.isArray(w.config.legend.markers.offsetY) ? parseFloat(w.config.legend.markers.offsetY[i2]) : parseFloat(w.config.legend.markers.offsetY);
    let mBorderWidth = Array.isArray(w.config.legend.markers.strokeWidth) ? parseFloat(w.config.legend.markers.strokeWidth[i2]) : parseFloat(w.config.legend.markers.strokeWidth);
  ...
  drawLegends() {
    ...
    for (let i2 = isLegendInversed ? legendNames.length - 1 : 0; isLegendInversed ? i2 >= 0 : i2 <= legendNames.length - 1; isLegendInversed ? i2-- : i2++) {
      let text = legendFormatter(legendNames[i2], { seriesIndex: i2, w });
      ...
      let elMarker = this.createLegendMarker({ i: i2, fillcolor });
      ...
      if (legendGroups.length) {
        w.globals.seriesGroups.forEach((group, gi) => {
          var _a2;
          if (group.includes((_a2 = w.config.series[i2]) == null ? void 0 : _a2.name)) {
      ...
      Graphics.setAttrs(elLegend, {
        rel: i2 + 1,
        seriesName: Utils$1.escapeString(legendNames[i2]),
        "data:collapsed": collapsedSeries || ancillaryCollapsedSeries
      });
      if (collapsedSeries || ancillaryCollapsedSeries) {
  ...
  setLegendWrapXY(offsetX, offsetY) {
    ...
    if (w.config.legend.position === "bottom") {
      y2 = w.globals.svgHeight - Math.min(legendHeight, w.globals.svgHeight / 2) - 5;
      ...
      y2 = (titleH > 0 ? titleH - 10 : 0) + (subtitleH > 0 ? subtitleH - 10 : 0);
    }
    elLegendWrap.style.position = "absolute";
    x = x + offsetX + w.config.legend.offsetX;
    y2 = y2 + offsetY + w.config.legend.offsetY;
    elLegendWrap.style.left = x + "px";
    elLegendWrap.style.top = y2 + "px";
  ...
  onLegendHovered(e2) {
    const w = this.w;
    const hoverOverLegend = e2.target.classList.contains("apexcharts-legend-series") || e2.target.classList.contains("apexcharts-legend-text") || e2.target.classList.contains("apexcharts-legend-marker");
    if (w.config.chart.type !== "heatmap" && !this.isBarsDistributed) {
      if (!e2.target.classList.contains("apexcharts-inactive-legend") && hoverOverLegend) {
        let series = new Series(this.ctx);
        series.toggleSeriesOnHover(e2, e2.target);
      }
    } else {
      if (hoverOverLegend) {
        let seriesCnt = parseInt(e2.target.getAttribute("rel"), 10) - 1;
        this.ctx.events.fireEvent("legendHover", [this.ctx, seriesCnt, this.w]);
        let series = new Series(this.ctx);
        series.highlightRangeInSeries(e2, e2.target);
      }
    }
  }
  onLegendKeyDown(e2) {
    const me = this;
    const isLegendItem = e2.target.classList.contains("apexcharts-legend-series") || e2.target.classList.contains("apexcharts-legend-text") || e2.target.classList.contains("apexcharts-legend-marker");
  ...
  onLegendClick(e2) {
    ...
    if (e2.target.classList.contains("apexcharts-legend-series") || e2.target.classList.contains("apexcharts-legend-text") || e2.target.classList.contains("apexcharts-legend-marker")) {
      let seriesCnt = parseInt(e2.target.getAttribute("rel"), 10) - 1;
      let isHidden = e2.target.getAttribute("data:collapsed") === "true";
      ...
      if (typeof markerClick === "function" && e2.target.classList.contains("apexcharts-legend-marker")) {
...
var Toolbar = class {
  ...
  createToolbar() {
    ...
    toolbarControls.forEach((t2, index) => {
      if (t2.index) {
        Utils$1.moveIndexInArray(toolbarControls, index, t2.index);
      }
    });
...
var SVGElement = class _SVGElement {
  ...
  addTo(parent) {
    const p2 = parent.node || parent;
    p2.appendChild(this.node);
    return this;
  }
  remove() {
...
var SVGGradient = class extends SVGElement {
  ...
  stop(offset, color, opacity) {
    const s2 = BrowserAPIs.createElementNS(SVGNS, "stop");
    s2.setAttribute("offset", offset);
    s2.setAttribute("stop-color", color);
    if (opacity !== void 0) s2.setAttribute("stop-opacity", opacity);
    this.node.appendChild(s2);
    return this;
  }
  from(x, y2) {
    return this.attr({ x1: x, y1: y2 });
  }
  to(x, y2) {
    return this.attr({ x2: x, y2 });
  }
  url() {
    return "url(#" + this._id + ")";
  }
...
var SVGFilter = class extends SVGElement {
  ...
  size(w, h2, x, y2) {
    return this.attr({ width: w, height: h2, x, y: y2 });
  }
};
...
function arcToBezier(pos, val) {
  var rx = Math.abs(val[1]), ry = Math.abs(val[2]), xAxisRotation = val[3] % 360, largeArcFlag = val[4], sweepFlag = val[5], x = val[6], y2 = val[7], A = new Point(pos[0], pos[1]), B = new Point(x, y2), primedCoord, lambda, mat, k, c2, cSquare, t2, O, OA, OB, tetaStart, tetaEnd, deltaTeta, nbSectors, f2, arcSegPoints, angle, sinAngle, cosAngle, pt, i2, il, retVal = [], x1, y1, x2, y22;
  ...
  f2 = 4 * Math.tan(deltaTeta / 4) / 3;
  for (i2 = 0; i2 <= nbSectors; i2++) {
    cosAngle = Math.cos(angle);
    sinAngle = Math.sin(angle);
    pt = new Point(O.x + cosAngle, O.y + sinAngle);
    arcSegPoints[i2] = [
      new Point(pt.x + f2 * sinAngle, pt.y - f2 * cosAngle),
      pt,
      new Point(pt.x - f2 * sinAngle, pt.y + f2 * cosAngle)
    ];
    angle += deltaTeta;
  ...
  for (i2 = 1, il = arcSegPoints.length; i2 < il; i2++) {
    ...
    x = pt.x;
    y2 = pt.y;
    retVal.push(["C", x1, y1, x2, y22, x, y2]);
  }
  return retVal;
}
  ...
  while (true) {
    ...
    if (startArrTemp[i2][0] != destArrTemp[i2][0] || startArrTemp[i2][0] == "M" || startArrTemp[i2][0] == "A" && (startArrTemp[i2][4] != destArrTemp[i2][4] || startArrTemp[i2][5] != destArrTemp[i2][5])) {
      Array.prototype.splice.apply(
        startArrTemp,
        [i2, 1].concat(toBezier.call(posStart, startArrTemp[i2]))
      );
      Array.prototype.splice.apply(
        destArrTemp,
        [i2, 1].concat(toBezier.call(posDest, destArrTemp[i2]))
      );
    } else {
      startArrTemp[i2] = setPosAndReflection.call(posStart, startArrTemp[i2]);
      destArrTemp[i2] = setPosAndReflection.call(posDest, destArrTemp[i2]);
    }
...
function installSelectable(ElementClass) {
  ElementClass.prototype.select = function(opts) {
    ...
    const updatePositions = () => {
      ...
      const y2 = parseFloat(el.attr("y")) || 0;
...
var ZoomPanSelection = class extends Toolbar {
  ...
  executeMouseWheelZoom(e2) {
    ...
    const mouseX = (e2.clientX - gridRectDim.left) / gridRectDim.width;
  ...
  drawSelectionRect({ x, y: y2, width, height, translateX = 0, translateY = 0 }) {
    const w = this.w;
...
var Utils2 = class {
  ...
  getNearestValues({ hoverArea, elGrid, clientX, clientY }) {
    ...
    let seriesXValArr = w.globals.seriesXvalues.map((seriesXVal) => {
      return seriesXVal.filter((s2) => Utils$1.isNumber(s2));
    });
    let seriesYValArr = w.globals.seriesYvalues.map((seriesYVal) => {
      return seriesYVal.filter((s2) => Utils$1.isNumber(s2));
    });
    if (w.globals.isXNumeric) {
  ...
  closestInArray(val, arr) {
    ...
    for (let i2 = 0; i2 < arr.length; i2++) {
      let newdiff = Math.abs(val - arr[i2]);
      if (newdiff < diff) {
        diff = newdiff;
        currIndex = i2;
      }
  ...
  isXoverlap(j) {
    ...
    const seriesX = w.globals.seriesX.filter((s2) => typeof s2[0] !== "undefined");
    if (seriesX.length > 0) {
  ...
  getAllMarkers(filterCollapsed = false) {
    ...
    markersWraps.sort((a2, b2) => {
      var indexA = Number(a2.getAttribute("data:realIndex"));
      var indexB = Number(b2.getAttribute("data:realIndex"));
...
var Labels = class {
  ...
  printLabels({ i: i2, j, values, ttItems, shared, e: e2 }) {
    ...
    let seriesName = "";
    let pColor = w.globals.colors[i2];
    if (j !== null && w.config.plotOptions.bar.distributed) {
      pColor = w.globals.colors[j];
    }
    for (let t2 = 0, inverset = w.globals.series.length - 1; t2 < w.globals.series.length; t2++, inverset--) {
      let f2 = this.getFormatters(i2);
      seriesName = this.getSeriesName({
        fn: f2.yLbTitleFormatter,
        index: i2,
        seriesIndex: i2,
        j
      });
      if (w.config.chart.type === "treemap") {
        seriesName = f2.yLbTitleFormatter(String(w.config.series[i2].data[j].x), {
          series: w.globals.series,
          seriesIndex: i2,
          dataPointIndex: j,
          w
      ...
      const tIndex = w.config.tooltip.inverseOrder ? inverset : t2;
      if (w.globals.axisCharts) {
        const getValBySeriesIndex = (index) => {
          var _a2, _b, _c, _d;
          if (w.globals.isRangeData) {
            return f2.yLbFormatter((_b = (_a2 = w.globals.seriesRangeStart) == null ? void 0 : _a2[index]) == null ? void 0 : _b[j], {
              series: w.globals.seriesRangeStart,
              seriesIndex: index,
              dataPointIndex: j,
              w
            }) + " - " + f2.yLbFormatter((_d = (_c = w.globals.seriesRangeEnd) == null ? void 0 : _c[index]) == null ? void 0 : _d[j], {
          ...
          return f2.yLbFormatter(w.globals.series[index][j], {
            series: w.globals.series,
            seriesIndex: index,
            dataPointIndex: j,
            w
        ...
        if (shared) {
          ...
          if (hasGoalValues(tIndex)) {
            goalVals = w.globals.seriesGoals[tIndex][j].map((goal) => {
              return {
                attrs: goal,
                val: f2.yLbFormatter(goal.value, {
                  seriesIndex: tIndex,
                  dataPointIndex: j,
                  w
                })
          ...
          val = getValBySeriesIndex(i2);
          if (hasGoalValues(i2) && Array.isArray(w.globals.seriesGoals[i2][j])) {
            goalVals = w.globals.seriesGoals[i2][j].map((goal) => {
              return {
                attrs: goal,
                val: f2.yLbFormatter(goal.value, {
                  seriesIndex: i2,
      ...
      if (j === null) {
        val = f2.yLbFormatter(w.globals.series[i2], __spreadProps(__spreadValues({}, w), {
          seriesIndex: i2,
          dataPointIndex: i2
        }));
      }
  ...
  getFormatters(i2) {
    const w = this.w;
    let yLbFormatter = w.globals.yLabelFormatters[i2];
    let yLbTitleFormatter;
    if (w.globals.ttVal !== void 0) {
      if (Array.isArray(w.globals.ttVal)) {
        yLbFormatter = w.globals.ttVal[i2] && w.globals.ttVal[i2].formatter;
        yLbTitleFormatter = w.globals.ttVal[i2] && w.globals.ttVal[i2].title && w.globals.ttVal[i2].title.formatter;
      } else {
  ...
  DOMHandling({ t: t2, j, ttItems, values, seriesName, shared, pColor }) {
    ...
    if (goalVals.length && w.globals.seriesGoals[t2]) {
      ...
      if (shared) {
        if (w.globals.seriesGoals[t2][j] && Array.isArray(w.globals.seriesGoals[t2][j])) {
          createGoalsHtml();
        } else {
          ttGLabel.innerHTML = "";
          ttGVal.innerHTML = "";
    ...
    if (shared && ttItemsChildren[0]) {
      if (w.config.tooltip.hideEmptySeries) {
        let ttItemMarker = ttItems[t2].querySelector(
          ".apexcharts-tooltip-marker"
        );
        let ttItemText = ttItems[t2].querySelector(".apexcharts-tooltip-text");
        if (parseFloat(val) == 0) {
  ...
  getValuesToPrint({ i: i2, j }) {
    ...
    if (j === null) {
      ...
      if (w.globals.isXNumeric && w.config.chart.type !== "treemap") {
        ...
        const dataFormat = new Data(this.ctx);
        if (dataFormat.isFormatXY()) {
          xVal = typeof w.config.series[i2].data[j] !== "undefined" ? w.config.series[i2].data[j].x : "";
        } else {
    ...
    if (w.globals.seriesZ.length > 0 && w.globals.seriesZ[i2].length > 0) {
      zVal = zFormatter(w.globals.seriesZ[i2][j], w);
    }
    if (typeof w.config.xaxis.tooltip.formatter === "function") {
...
var Position = class {
  ...
  moveTooltip(cx, cy, markerSize = null) {
    ...
    let y2 = parseFloat(cy) + pointSize / 2;
    ...
    if (w.config.tooltip.followCursor) {
      ...
      y2 = ttCtx.e.clientY + w.globals.translateY - seriesBound.top;
      if (y2 > w.globals.gridHeight / 2) {
        y2 = y2 - ttCtx.tooltipRect.ttHeight;
      }
    } else {
      if (!w.globals.isBarHorizontal) {
        if (tooltipRect.ttHeight / 2 + y2 > w.globals.gridHeight) {
          y2 = w.globals.gridHeight - tooltipRect.ttHeight + w.globals.translateY;
        }
      }
    }
    if (!isNaN(x)) {
  ...
  moveMarkers(i2, j) {
    ...
    if (w.globals.markers.size[i2] > 0) {
      ...
      for (let p2 = 0; p2 < allPoints.length; p2++) {
        if (parseInt(allPoints[p2].getAttribute("rel"), 10) === j) {
          ttCtx.marker.resetPointsSize();
          ttCtx.marker.enlargeCurrentPoint(j, allPoints[p2]);
        }
      }
    } else {
      ttCtx.marker.resetPointsSize();
  ...
  moveDynamicPointsOnHover(j) {
    ...
    if (points.length) {
      for (let p2 = 0; p2 < w.globals.series.length; p2++) {
        let pointArr = pointsArr[p2];
        if (w.globals.comboCharts) {
          if (typeof pointArr === "undefined") {
            points.splice(p2, 0, null);
          }
        }
        if (pointArr && pointArr.length) {
          let pcy = pointsArr[p2][j][1];
          let pcy2;
          points[p2].setAttribute("cx", cx);
          const shape = points[p2].getAttribute("shape");
          if (w.config.chart.type === "rangeArea" && !w.globals.comboCharts) {
            const rangeStartIndex = j + w.globals.series[p2].length;
            pcy2 = pointsArr[p2][rangeStartIndex][1];
          ...
          if (pcy !== null && !isNaN(pcy) && pcy < w.globals.gridHeight + hoverSize && pcy + hoverSize > 0) {
            ...
            points[p2].setAttribute("d", path);
          } else {
            points[p2].setAttribute("d", "");
          }
        }
      }
    }
  ...
  moveStickyTooltipOverBars(j, capturedSeries) {
    ...
    let jBar = w.globals.dom.baseEl.querySelector(
      `.apexcharts-bar-series .apexcharts-series[rel='${i2}'] path[j='${j}'], .apexcharts-candlestick-series .apexcharts-series[rel='${i2}'] path[j='${j}'], .apexcharts-boxPlot-series .apexcharts-series[rel='${i2}'] path[j='${j}'], .apexcharts-rangebar-series .apexcharts-series[rel='${i2}'] path[j='${j}']`
    );
...
var Marker = class {
  ...
  drawDynamicPoints() {
    ...
    elsSeries = [...elsSeries];
    if (w.config.chart.stacked) {
      elsSeries.sort((a2, b2) => {
        return parseFloat(a2.getAttribute("data:realIndex")) - parseFloat(b2.getAttribute("data:realIndex"));
      });
    }
    for (let i2 = 0; i2 < elsSeries.length; i2++) {
      let pointsMain = elsSeries[i2].querySelector(
        `.apexcharts-series-markers-wrap`
      );
      if (pointsMain !== null) {
        let point;
  ...
  enlargeCurrentPoint(rel, point, x = null, y2 = null) {
    let w = this.w;
    if (w.config.chart.type !== "bubble") {
  ...
  enlargePoints(j) {
    ...
    for (let p2 = 0; p2 < points.length; p2++) {
      let rel = points[p2].getAttribute("rel");
      let index = points[p2].getAttribute("index");
      if (newSize === void 0) {
        newSize = w.globals.markers.size[index] + w.config.markers.hover.sizeOffset;
      }
      if (col === parseInt(rel, 10)) {
        me.newPointSize(col, points[p2]);
        let cx = points[p2].getAttribute("cx");
        let cy = points[p2].getAttribute("cy");
        me.tooltipPosition.moveXCrosshairs(cx);
        if (!ttCtx.fixedTooltip) {
          me.tooltipPosition.moveTooltip(cx, cy, newSize);
        }
      } else {
        me.oldPointSize(points[p2]);
      }
    }
  }
  ...
  resetPointsSize() {
    ...
    for (let p2 = 0; p2 < points.length; p2++) {
      const size = parseFloat(points[p2].getAttribute("default-marker-size"));
      if (Utils$1.isNumber(size) && size > 0) {
        const path = this.ttCtx.tooltipUtil.getPathFromPoint(points[p2], size);
        points[p2].setAttribute("d", path);
      } else {
        points[p2].setAttribute("d", "M0,0");
      }
    }
  }
};
var Intersect = class {
  ...
  getAttr(e2, attr) {
    return parseFloat(e2.target.getAttribute(attr));
  }
  // handle tooltip for heatmaps and treemaps
  handleHeatTreeTooltip({ e: e2, opt, x, y: y2, type }) {
    const ttCtx = this.ttCtx;
    const w = this.w;
    if (e2.target.classList.contains(`apexcharts-${type}-rect`)) {
      let i2 = this.getAttr(e2, "i");
      let j = this.getAttr(e2, "j");
      let cx = this.getAttr(e2, "cx");
      let cy = this.getAttr(e2, "cy");
      let width = this.getAttr(e2, "width");
      let height = this.getAttr(e2, "height");
      ...
      w.globals.capturedSeriesIndex = i2;
      w.globals.capturedDataPointIndex = j;
      x = cx + ttCtx.tooltipRect.ttWidth / 2 + width;
      y2 = cy + ttCtx.tooltipRect.ttHeight / 2 - height / 2;
      ...
      if (ttCtx.w.config.tooltip.followCursor) {
        ...
        y2 = w.globals.clientY - seriesBound.top - (y2 > w.globals.gridHeight / 2 ? ttCtx.tooltipRect.ttHeight : 0);
      }
    }
    return {
      x,
   ...
   */
  handleMarkerTooltip({ e: e2, opt, x, y: y2 }) {
    let w = this.w;
    const ttCtx = this.ttCtx;
    let i2;
    let j;
    if (e2.target.classList.contains("apexcharts-marker")) {
      ...
      w.globals.capturedDataPointIndex = j;
      x = cx;
      y2 = cy + w.globals.translateY - ttCtx.tooltipRect.ttHeight * 1.4;
      if (ttCtx.w.config.tooltip.followCursor) {
        const elGrid = ttCtx.getElGrid();
        const seriesBound = elGrid.getBoundingClientRect();
        y2 = ttCtx.e.clientY + w.globals.translateY - seriesBound.top;
      ...
      ttCtx.marker.enlargeCurrentPoint(j, opt.paths, x, y2);
    }
    return {
      x,
      y: y2
  ...
  handleBarTooltip({ e: e2, opt }) {
    ...
    if (isNaN(y2)) {
      y2 = w.globals.svgHeight - ttCtx.tooltipRect.ttHeight;
    }
    ...
    if (ttCtx.w.config.tooltip.followCursor) {
      const elGrid = ttCtx.getElGrid();
      const seriesBound = elGrid.getBoundingClientRect();
      y2 = ttCtx.e.clientY - seriesBound.top;
    }
    ...
    if (!ttCtx.fixedTooltip && (!w.config.tooltip.shared || w.globals.isBarHorizontal && ttCtx.tooltipUtil.hasBars())) {
      y2 = y2 + w.globals.translateY - ttCtx.tooltipRect.ttHeight / 2;
  ...
  getBarTooltipXY({ e: e2, opt }) {
    ...
    if (cl.contains("apexcharts-bar-area") || cl.contains("apexcharts-candlestick-area") || cl.contains("apexcharts-boxPlot-area") || cl.contains("apexcharts-rangebar-area")) {
      ...
      let y22 = bar.getAttribute("data-range-y2");
      if (w.globals.comboCharts) {
      ...
      if (w.config.tooltip.followCursor) {
        if (w.globals.isBarHorizontal) {
          ...
          y2 = e2.clientY - seriesBound.top - ttCtx.tooltipRect.ttHeight / 2 - 15;
        }
      } else {
        if (w.globals.isBarHorizontal) {
          x = cx;
...
var AxesTooltip = class {
  ...
  drawYaxisTooltip() {
    ...
    for (let i2 = 0; i2 < w.config.yaxis.length; i2++) {
      ...
      let tooltipCssClass = isRight ? `apexcharts-yaxistooltip apexcharts-yaxistooltip-${i2} apexcharts-yaxistooltip-right` : `apexcharts-yaxistooltip apexcharts-yaxistooltip-${i2} apexcharts-yaxistooltip-left`;
      let renderTo = w.globals.dom.elWrap;
      ...
      if (yaxisTooltip === null) {
        ...
        ttCtx.yaxisTooltipText[i2] = document.createElement("div");
        ttCtx.yaxisTooltipText[i2].classList.add("apexcharts-yaxistooltip-text");
        ttCtx.yaxisTooltip.appendChild(ttCtx.yaxisTooltipText[i2]);
      }
    }
  }
  /**
...
var Tooltip = class {
  ...
  drawTooltip(xyRatios) {
    ...
    this.yaxisTooltips = w.config.yaxis.map((y2) => {
      return y2.show && y2.tooltip.enabled && w.globals.axisCharts ? true : false;
    });
  ...
  createTTElements(ttItemsCnt) {
    ...
    for (let i2 = 0; i2 < ttItemsCnt; i2++) {
      ...
      gTxt.style.order = w.config.tooltip.inverseOrder ? ttItemsCnt - i2 : i2 + 1;
  ...
  addSVGEvents() {
    ...
    if (w.globals.axisCharts) {
      ...
      if (points && points.length) {
        for (let p2 = 0; p2 < points.length; p2++) {
          tooltipY.push(points[p2].getAttribute("cy"));
          tooltipX.push(points[p2].getAttribute("cx"));
        }
      }
    }
  ...
  addPathsEventListeners(paths, opts) {
    ...
    for (let p2 = 0; p2 < paths.length; p2++) {
      ...
      events.map((ev) => {
        return paths[p2].addEventListener(
          ev,
  ...
  axisChartsTooltips({ e: e2, opt }) {
    ...
    if (e2.type === "mousemove" || e2.type === "touchmove" || e2.type === "mouseup") {
      ...
      const hasYAxisTooltip = this.yaxisTooltips.filter((b2) => {
        return b2 === true;
      });
  ...
  nonAxisChartsTooltips({ e: e2, opt, tooltipRect }) {
    ...
    if (e2.type === "mousemove" || e2.type === "touchmove") {
      ...
      let y2 = w.globals.clientY - seriesBound.top - tooltipRect.ttHeight - 10;
      tooltipEl.style.left = x + "px";
      tooltipEl.style.top = y2 + "px";
      if (w.config.legend.tooltipHoverFormatter) {
        let legendFormatter = w.config.legend.tooltipHoverFormatter;
        const i2 = rel - 1;
        const legendName = this.legendLabels[i2].getAttribute("data:default-text");
        let text = legendFormatter(legendName, {
          seriesIndex: i2,
          dataPointIndex: i2,
          w
        });
        this.legendLabels[i2].innerHTML = text;
      }
    } else if (e2.type === "mouseout" || e2.type === "touchend") {
      tooltipEl.classList.remove("apexcharts-active");
      w.globals.dom.baseEl.classList.remove("apexcharts-tooltip-active");
      if (w.config.legend.tooltipHoverFormatter) {
        this.legendLabels.forEach((l2) => {
          const defaultText = l2.getAttribute("data:default-text");
          l2.innerHTML = decodeURIComponent(defaultText);
        });
      }
    }
  }
  handleStickyTooltip(e2, clientX, clientY, opt) {
    ...
    if (capturedSeries !== null) {
      this.handleStickyCapturedSeries(e2, capturedSeries, opt, j);
    } else {
  ...
  deactivateHoverFilter() {
    ...
    for (let b2 = 0; b2 < allPaths.length; b2++) {
      graphics.pathMouseLeave(allPaths[b2]);
    }
  }
  handleMouseOut(opt) {
    const w = this.w;
  ...
  create(e2, context, capturedSeries, j, ttItems, shared = null) {
    ...
    if (w.config.legend.tooltipHoverFormatter) {
      ...
      els.forEach((l2) => {
        ...
        l2.innerHTML = decodeURIComponent(legendName);
      });
      for (let i2 = 0; i2 < els.length; i2++) {
        const l2 = els[i2];
        const lsIndex = parseInt(l2.getAttribute("i"), 10);
        const legendName = decodeURIComponent(
          l2.getAttribute("data:default-text")
        );
        let text = legendFormatter(legendName, {
    ...
    if (shared) {
      ...
      if (hasMarkers) {
        ...
        if (this.barSeriesHeight > 0) {
          ...
          for (let b2 = 0; b2 < paths.length; b2++) {
            graphics.pathMouseEnter(paths[b2]);
          }
        }
      }
    } else {
...
var TimeScale = class {
  ...
  generateYearScale({
    ...
    for (let i2 = 0; i2 < numberOfYears; i2++) {
      year++;
  ...
  generateMonthScale({
    ...
    let pos = firstTickPosition;
    for (let i2 = 0, j = 1; i2 < numberOfMonths; i2++, j++) {
      month = Utils$1.monthMod(month);
      if (month === 0) {
        unit = "year";
        yrCounter += 1;
  ...
  generateDayScale({
    ...
    let pos = firstTickPosition;
    for (let i2 = 0; i2 < numberOfDays; i2++) {
      date += 1;
      unit = "day";
      month = changeMonth(
        date,
  ...
  generateHourScale({
    ...
    hour++;
    let pos = firstTickPosition;
    for (let i2 = 0; i2 < numberOfHours; i2++) {
      unit = "hour";
      if (hour >= 24) {
        hour = 0;
        date += 1;
  ...
  generateMinuteScale({
    ...
    let pos = firstTickPosition;
    for (let i2 = 0; i2 < numberOfMinutes; i2++) {
      if (minute >= 60) {
        minute = 0;
        hour += 1;
        if (hour === 24) {
  ...
  generateSecondScale({
    ...
    let hour = currentHour;
    let pos = firstTickPosition;
    for (let i2 = 0; i2 < numberOfSeconds; i2++) {
      if (second >= 60) {
        minute++;
        second = 0;
        if (minute >= 60) {
  ...
  removeOverlappingTS(arr) {
    ...
    filteredArray = filteredArray.filter((f2) => f2 !== null);
...
var BarDataLabels = class {
  ...
  handleBarDataLabels(opts) {
    ...
    if (w.globals.isXNumeric && !w.globals.isBarHorizontal) {
      ...
      bcy = y2 + parseFloat(barHeight * (visibleSeries + 1)) - strokeWidth;
    } else {
      bcx = x + parseFloat(barWidth * visibleSeries);
      bcy = y2 + parseFloat(barHeight * visibleSeries);
    }
    let dataLabels = null;
    ...
    if (w.config.dataLabels.enabled) {
      const yLabel = w.globals.series[i2][j];
    ...
    dataLabels = this.drawCalculatedDataLabels({
      ...
      val: this.barCtx.isRangeBar ? [y1, y22] : w.config.chart.stackType === "100%" ? series[realIndex][j] : w.globals.series[realIndex][j],
      i: realIndex,
      j,
      barWidth,
      barHeight,
  ...
  calculateColumnsDataLabelsPosition(opts) {
    ...
    let valIsNegative = w.globals.series[i2][j] < 0;
    let newY = y2;
    if (this.barCtx.isReversed) {
      newY = y2 + (valIsNegative ? barHeight : -barHeight);
    }
  ...
  drawCalculatedDataLabels({
    ...
    if (w.config.plotOptions.bar.dataLabels.orientation === "vertical")
      rotate = `rotate(-90, ${x}, ${y2})`;
    const dataLabels = new DataLabels(this.barCtx.ctx);
    const graphics = new Graphics(this.barCtx.ctx);
    const formatter = dataLabelsConfig.formatter;
    let elDataLabelsWrap = null;
    const isSeriesNotCollapsed = w.globals.collapsedSeriesIndices.indexOf(i2) > -1;
  ...
  drawTotalDataLabels({
    ...
    if (barTotalDataLabelsConfig.enabled && typeof x !== "undefined" && typeof y2 !== "undefined" && this.barCtx.lastActiveBarSerieIndex === realIndex) {
...
var Helpers$1 = class Helpers4 {
  ...
  initialPositions(realIndex) {
    let w = this.w;
    let x, y2, yDivision, xDivision, barHeight, barWidth, zeroH, zeroW;
    ...
    if (this.barCtx.isHorizontal) {
      ...
      y2 = (yDivision - barHeight * this.barCtx.seriesLen) / 2;
    } else {
  ...
  getPathFillColor(series, i2, j, realIndex) {
    var _a, _b, _c, _d;
    const w = this.w;
    let fill = this.barCtx.ctx.fill;
    let fillColor = null;
    let seriesNumber = this.barCtx.barOptions.distributed ? j : i2;
    let useRangeColor = false;
    if (this.barCtx.barOptions.colors.ranges.length > 0) {
      const colorRange = this.barCtx.barOptions.colors.ranges;
      colorRange.map((range) => {
        if (series[i2][j] >= range.from && series[i2][j] <= range.to) {
    ...
    let pathFill = fill.fillPath({
      ...
      color: fillColor,
      value: series[i2][j],
      fillConfig: (_a = w.config.series[i2].data[j]) == null ? void 0 : _a.fill,
      fillType: ((_c = (_b = w.config.series[i2].data[j]) == null ? void 0 : _b.fill) == null ? void 0 : _c.type) ? (_d = w.config.series[i2].data[j]) == null ? void 0 : _d.fill.type : Array.isArray(w.config.fill.type) ? w.config.fill.type[realIndex] : w.config.fill.type
    });
    return {
      color: pathFill,
      useRangeColor
  ...
  getStrokeWidth(i2, j, realIndex) {
    let strokeWidth = 0;
    const w = this.w;
    if (typeof this.barCtx.series[i2][j] === "undefined" || this.barCtx.series[i2][j] === null || w.config.chart.type === "bar" && !this.barCtx.series[i2][j]) {
  ...
  createBorderRadiusArr(series) {
    ...
    for (let j = 0; j < numColumns; j++) {
      ...
      for (let i2 = 0; i2 < numSeries; i2++) {
        const value = series[i2][j];
        if (value > 0) {
          positiveIndices.push(i2);
          nonZeroCount++;
        } else if (value < 0) {
          negativeIndices.push(i2);
          nonZeroCount++;
        }
      }
      if (positiveIndices.length > 0 && negativeIndices.length === 0) {
        if (positiveIndices.length === 1) {
          ...
          for (let i2 of positiveIndices) {
            if (i2 === firstPositiveIndex) {
              output[i2][j] = chartType === "bar" && numColumns === 1 ? "top" : "bottom";
            } else if (i2 === lastPositiveIndex) {
              output[i2][j] = "top";
            } else {
              output[i2][j] = "none";
            }
        ...
        if (negativeIndices.length === 1) {
          ...
          for (let i2 of negativeIndices) {
            if (i2 === highestNegativeIndex) {
              output[i2][j] = "bottom";
            } else if (i2 === lowestNegativeIndex) {
              output[i2][j] = "top";
            } else {
              output[i2][j] = "none";
            }
  ...
  barBackground({ j, i: i2, x1, x2, y1, y2, elSeries }) {
    const w = this.w;
    const graphics = new Graphics(this.barCtx.ctx);
    const sr = new Series(this.barCtx.ctx);
    let activeSeriesIndex = sr.getActiveConfigSeriesIndex();
    if (this.barCtx.barOptions.colors.backgroundBarColors.length > 0 && activeSeriesIndex === i2) {
  ...
  getColumnPaths({
    ...
    let direction = (series[i2][j] >= 0 ? 1 : -1) * (isReversed ? -1 : 1);
    ...
    if (w.config.chart.stacked) {
      ...
      _ctx.yArrjVal.push(this.barCtx.series[i2][j]);
    }
    return {
      pathTo,
      pathFrom
  ...
  getBarpaths({
    ...
    if (w.config.chart.stacked) {
      ...
      _ctx.xArrjVal.push(this.barCtx.series[i2][j]);
    }
    return {
      pathTo,
      pathFrom
  ...
  getGoalValues(type, zeroW, zeroH, i2, j, translationsIndex) {
    const w = this.w;
    let goals = [];
    ...
    if (w.globals.seriesGoals[i2] && w.globals.seriesGoals[i2][j] && Array.isArray(w.globals.seriesGoals[i2][j])) {
      w.globals.seriesGoals[i2][j].forEach((goal) => {
    ...
    if (this.barCtx.barOptions.isDumbbell && w.globals.seriesRange.length) {
      ...
      const commonAttrs = {
        strokeHeight: type === "x" ? 0 : w.globals.markers.size[i2],
        strokeWidth: type === "x" ? w.globals.markers.size[i2] : 0,
        strokeDashArray: 0,
        strokeLineCap: "round",
        strokeColor: Array.isArray(colors[i2]) ? colors[i2][0] : colors[i2]
      };
      pushGoal(w.globals.seriesRangeStart[i2][j], commonAttrs);
      pushGoal(w.globals.seriesRangeEnd[i2][j], __spreadProps(__spreadValues({}, commonAttrs), {
        strokeColor: Array.isArray(colors[i2]) ? colors[i2][1] : colors[i2]
      }));
    }
    return goals;
  }
      ...
      if (Array.isArray(goalX)) {
        goalX.forEach((goal) => {
          if (goal.x >= -1 && goal.x <= graphics.w.globals.gridWidth + 1) {
            ...
            let y2 = barYPosition + sHeight + barHeight / 2;
...
var Bar = class {
  constructor(ctx, xyRatios) {
    ...
    this.stackedSeriesTotals = coreUtils.getStackedSeriesTotals(
      this.w.config.series.map((s2, i2) => {
        return barSeriesIndices.indexOf(i2) === -1 ? i2 : -1;
      }).filter((s2) => {
        return s2 !== -1;
      })
    );
  ...
  draw(series, seriesIndex) {
    ...
    for (let i2 = 0, bc = 0; i2 < series.length; i2++, bc++) {
      let x, y2, xDivision, yDivision, zeroH, zeroW;
      let yArrj = [];
      let xArrj = [];
      let realIndex = w.globals.comboCharts ? seriesIndex[i2] : i2;
      ...
      for (let j = 0; j < series[i2].length; j++) {
        const strokeWidth = this.barHelpers.getStrokeWidth(i2, j, realIndex);
        let paths = null;
        const pathsParams = {
          indexes: {
            i: i2,
        ...
        if (this.isHorizontal) {
          ...
          barWidth = this.series[i2][j] / this.invertedYRatio;
        } else {
          ...
          barHeight = this.series[i2][j] / this.yRatio[translationsIndex];
        }
        let pathFill = this.barHelpers.getPathFillColor(series, i2, j, realIndex);
        if (this.isFunnel && this.barOptions.isFunnel3d && this.pathArr.length && j > 0) {
          const barShadow = this.barHelpers.drawBarShadow({
            color: typeof pathFill.color === "string" && ((_a = pathFill.color) == null ? void 0 : _a.indexOf("url")) === -1 ? pathFill.color : Utils$1.hexToRgba(w.globals.colors[i2]),
  ...
  renderSeries({
    ...
    if (w.config.series[i2].data[j] && w.config.series[i2].data[j].strokeColor) {
      lineFill = w.config.series[i2].data[j].strokeColor;
    }
    if (this.isNullValue) {
      pathFill = "none";
    }
    ...
    if (!skipDrawing) {
      ...
      if (typeof y1 !== "undefined" && typeof y22 !== "undefined") {
        renderedPath.attr("data-range-y1", y1);
        renderedPath.attr("data-range-y2", y22);
      }
      const filters = new Filters(this.ctx);
  ...
  drawBarPaths({
    ...
    let barYPosition;
    if (w.globals.isXNumeric) {
      y2 = (w.globals.seriesX[i2][j] - w.globals.minX) / this.invertedXRatio - barHeight;
      barYPosition = y2 + barHeight * this.visibleI;
    } else {
      if (w.config.plotOptions.bar.hideZeroBarsWhenGrouped) {
        const { nonZeroColumns, zeroEncounters } = this.barHelpers.getZeroValueEncounters({ i: i2, j });
        if (nonZeroColumns > 0) {
    ...
    if (this.isFunnel) {
      zeroW = zeroW - (this.barHelpers.getXForValue(this.series[i2][j], zeroW) - zeroW) / 2;
    }
    x = this.barHelpers.getXForValue(this.series[i2][j], zeroW);
    ...
    return {
      ...
      x,
      y: y2,
      goalX: this.barHelpers.getGoalValues("x", zeroW, null, i2, j),
      barYPosition,
      barHeight
    };
  }
  drawColumnPaths({
    ...
    y2 = this.barHelpers.getYForValue(
      this.series[i2][j],
      zeroH,
      translationsIndex
    );
...
var BarStacked = class extends Bar {
    ...
    for (let i2 = 0, bc = 0; i2 < series.length; i2++, bc++) {
      ...
      for (let j = 0; j < w.globals.dataPoints; j++) {
        ...
        const commonPathOpts = {
          indexes: { i: i2, j, realIndex, translationsIndex, bc },
          strokeWidth,
          x,
          y: y2,
          elSeries,
  ...
  initialPositions(x, y2, xDivision, yDivision, zeroH, zeroW, translationsIndex) {
    let w = this.w;
  ...
  drawStackedBarPaths({
    ...
    let w = this.w;
    let barYPosition = y2 + columnGroupIndex * barHeight;
    let barXPosition;
    let i2 = indexes.i;
    let j = indexes.j;
    let realIndex = indexes.realIndex;
    ...
    if (gsi > 0) {
      ...
      if (this.groupCtx.prevXVal[gsi - 1][j] < 0) {
        bXP = this.series[i2][j] >= 0 ? this.groupCtx.prevX[gsi - 1][j] + prevBarW - (this.isReversed ? prevBarW : 0) * 2 : this.groupCtx.prevX[gsi - 1][j];
      } else if (this.groupCtx.prevXVal[gsi - 1][j] >= 0) {
        bXP = this.series[i2][j] >= 0 ? this.groupCtx.prevX[gsi - 1][j] : this.groupCtx.prevX[gsi - 1][j] - prevBarW + (this.isReversed ? prevBarW : 0) * 2;
      }
      barXPosition = bXP;
    } else {
      barXPosition = zeroW;
    }
    if (this.series[i2][j] === null) {
      x = barXPosition;
    } else {
      x = barXPosition + this.series[i2][j] / this.invertedYRatio - (this.isReversed ? this.series[i2][j] / this.invertedYRatio : 0) * 2;
    }
    const paths = this.barHelpers.getBarpaths({
      barYPosition,
      barHeight,
  ...
  drawStackedColumnPaths({
    ...
    if (gsi > 0 && !w.globals.isXNumeric || gsi > 0 && w.globals.isXNumeric && w.globals.seriesX[realIndex - 1][j] === w.globals.seriesX[realIndex][j]) {
      ...
      for (let ii = 1; ii < p2; ii++) {
        if (((_b = this.groupCtx.prevYVal[gsi - ii]) == null ? void 0 : _b[j]) < 0) {
          bYP = this.series[i2][j] >= 0 ? prevYValue - prevBarH + (this.isReversed ? prevBarH : 0) * 2 : prevYValue;
          break;
        } else if (((_c = this.groupCtx.prevYVal[gsi - ii]) == null ? void 0 : _c[j]) >= 0) {
          bYP = this.series[i2][j] >= 0 ? prevYValue : prevYValue + prevBarH - (this.isReversed ? prevBarH : 0) * 2;
          break;
        }
      }
    ...
    if (this.series[i2][j]) {
      y2 = barYPosition - this.series[i2][j] / this.yRatio[translationsIndex] + (this.isReversed ? this.series[i2][j] / this.yRatio[translationsIndex] : 0) * 2;
    } else {
      y2 = barYPosition;
    }
    const paths = this.barHelpers.getColumnPaths({
    ...
    return {
      pathTo: paths.pathTo,
      pathFrom: paths.pathFrom,
      goalY: this.barHelpers.getGoalValues("y", null, zeroH, i2, j),
      barXPosition,
...
var BoxCandleStick = class extends Bar {
  draw(series, ctype, seriesIndex) {
    ...
    for (let i2 = 0; i2 < series.length; i2++) {
      this.isBoxPlot = w.config.chart.type === "boxPlot" || w.config.series[i2].type === "boxPlot";
      let x, y2, xDivision, yDivision, zeroH, zeroW;
      const yArrj = [];
      const xArrj = [];
      const realIndex = w.globals.comboCharts ? seriesIndex[i2] : i2;
  ...
  drawVerticalBoxPaths({
    ...
    if (typeof this.series[i2][j] === "undefined" || this.series[i2][j] === null) {
      y1 = zeroH;
      y2 = zeroH;
    } else {
      y1 = zeroH - y1 / yRatio;
  ...
  drawHorizontalBoxPaths({
    ...
    let m = ohlc.m;
    if (w.globals.isXNumeric) {
      y2 = (w.globals.seriesX[realIndex][j] - w.globals.minX) / this.invertedXRatio - barHeight / 2;
    }
    const barYPosition = y2 + barHeight * this.visibleI;
    if (typeof this.series[i2][j] === "undefined" || this.series[i2][j] === null) {
      x1 = zeroW;
      x2 = zeroW;
    } else {
  ...
  getOHLCValue(i2, j) {
    ...
    const getCandleVal = (arr) => arr[i2] && arr[i2][j] != null ? coreUtils.getLogValAtSeriesIndex(arr[i2][j], i2) : 0;
    const h2 = getCandleVal(w.globals.seriesCandleH);
    const o2 = getCandleVal(w.globals.seriesCandleO);
    const m = getCandleVal(w.globals.seriesCandleM);
    const c2 = getCandleVal(w.globals.seriesCandleC);
    const l2 = getCandleVal(w.globals.seriesCandleL);
    return {
      o: this.isBoxPlot ? h2 : o2,
      h: this.isBoxPlot ? o2 : h2,
      m,
      l: this.isBoxPlot ? c2 : l2,
      c: this.isBoxPlot ? l2 : c2
    };
  }
};
var TreemapHelpers = class {
  ...
  getShadeColor(chartType, i2, j, negRange) {
    const w = this.w;
    let colorShadePercent = 1;
    let shadeIntensity = w.config.plotOptions[chartType].shadeIntensity;
    const colorProps = this.determineColor(chartType, i2, j);
  ...
  determineColor(chartType, i2, j) {
    const w = this.w;
    let val = w.globals.series[i2][j];
    let chartOpts = w.config.plotOptions[chartType];
    let seriesNumber = chartOpts.colorScale.inverse ? j : i2;
    if (chartOpts.distributed && w.config.chart.type === "treemap") {
      seriesNumber = j;
    }
    let color = w.globals.colors[seriesNumber];
    let foreColor = null;
    let min = Math.min(...w.globals.series[i2]);
    let max = Math.max(...w.globals.series[i2]);
  ...
  calculateDataLabels({ text, x, y: y2, i: i2, j, colorProps, fontSize }) {
    let w = this.w;
    let dataLabelsConfig = w.config.dataLabels;
    const graphics = new Graphics(this.ctx);
    ...
    if (dataLabelsConfig.enabled) {
      ...
      let dataLabelsY = y2 + parseFloat(dataLabelsConfig.style.fontSize) / 3 + offY;
      dataLabels.plotDataLabelsText({
        x: dataLabelsX,
        y: dataLabelsY,
        text,
...
var HeatMap = class {
  ...
  draw(series) {
    ...
    for (let i2 = rev ? 0 : heatSeries.length - 1; rev ? i2 < heatSeries.length : i2 >= 0; rev ? i2++ : i2--) {
      let elSeries = graphics.group({
        class: `apexcharts-series apexcharts-heatmap-series`,
        seriesName: Utils$1.escapeString(w.globals.seriesNames[i2]),
        rel: i2 + 1,
        "data:realIndex": i2
      });
      this.ctx.series.addCollapsedClassToSeries(elSeries, i2);
      graphics.setupEventDelegation(elSeries, ".apexcharts-heatmap-rect");
      if (w.config.chart.dropShadow.enabled) {
        const shadow = w.config.chart.dropShadow;
        const filters = new Filters(this.ctx);
        filters.dropShadow(elSeries, shadow, i2);
      }
      let x1 = 0;
      ...
      for (let dIndex = 0; dIndex < w.globals.dataPoints; dIndex++) {
        if (w.globals.seriesX.length && !w.globals.allSeriesHasEqualX) {
          if (w.globals.minX + w.globals.minXDiff * dIndex < w.globals.seriesX[i2][j]) {
            x1 = x1 + xDivision;
            continue;
          }
        }
        if (j >= heatSeries[i2].length) break;
        ...
        if (w.globals.dataChanged) {
          ...
          if (this.dynamicAnim.enabled && w.globals.shouldAnimate) {
            ...
            let colorFrom = w.globals.previousPaths[i2] && w.globals.previousPaths[i2][j] && w.globals.previousPaths[i2][j].color;
        ...
        let formattedText = formatter(w.globals.series[i2][j], {
  ...
  animateHeatMap(el, x, y2, width, height, speed) {
...
var Helpers5 = class {
  ...
  sameValueSeriesFix(i2, series) {
    const w = this.w;
    if (w.config.fill.type === "gradient" || w.config.fill.type[i2] === "gradient") {
      const coreUtils = new CoreUtils(this.lineCtx.ctx);
      if (coreUtils.seriesHaveSameValues(i2)) {
        let gSeries = series[i2].slice();
  ...
  calculatePoints({ series, realIndex, x, y: y2, i: i2, j, prevY }) {
    let w = this.w;
    let ptX = [];
    let ptY = [];
    ...
    ptY.push(
      Utils$1.isNumber(series[i2][j + 1]) ? y2 + w.config.markers.offsetY : null
    );
    return {
      x: ptX,
      y: ptY
  ...
  determineFirstPrevY({
    ...
    if (typeof ((_c = series[i2]) == null ? void 0 : _c[0]) !== "undefined") {
      if (stackSeries) {
        if (i2 > 0) {
          lineYPosition = this.lineCtx.prevSeriesY[i2 - 1][0];
        } else {
      ...
      prevY = lineYPosition - series[i2][0] / this.lineCtx.yRatio[translationsIndex] + (this.lineCtx.isReversed ? series[i2][0] / this.lineCtx.yRatio[translationsIndex] : 0) * 2;
    } else {
      if (stackSeries && i2 > 0 && typeof series[i2][0] === "undefined") {
        for (let s2 = i2 - 1; s2 >= 0; s2--) {
          if (series[s2][0] !== null && typeof series[s2][0] !== "undefined") {
            lineYPosition = this.lineCtx.prevSeriesY[s2][0];
            prevY = lineYPosition;
            break;
          }
        }
...
var tangents = (points) => {
  ...
  for (let i2 = 0; i2 <= n; i2++) {
    s2 = (points[Math.min(n, i2 + 1)][0] - points[Math.max(0, i2 - 1)][0]) / (6 * (1 + m[i2] * m[i2]));
...
var spline = {
  ...
  points(points) {
    ...
    for (let i2 = 2, n = tgts.length; i2 < n; i2++) {
      const p22 = points[i2];
      const t22 = tgts[i2];
      pts.push([p22[0] - t22[0], p22[1] - t22[1], p22[0], p22[1]]);
    }
    return pts;
  },
  /**
...
function finiteDifferences(points) {
  ...
  let i2 = 1;
  for (let n = points.length - 1; i2 < n; i2++) {
    p0 = p1;
    p1 = points[i2 + 1];
...
var Line = class {
  ...
  draw(series, ctype, seriesIndex, seriesRangeEnd) {
    ...
    let allSeries = [];
    for (let i2 = 0; i2 < series.length; i2++) {
      series = this.lineHelpers.sameValueSeriesFix(i2, series);
      let realIndex = w.globals.comboCharts ? seriesIndex[i2] : i2;
    ...
    if (typeof ((_a = w.config.series[0]) == null ? void 0 : _a.zIndex) !== "undefined") {
      allSeries.sort(
        (a2, b2) => Number(a2.node.getAttribute("zIndex")) - Number(b2.node.getAttribute("zIndex"))
      );
    }
    if (w.config.chart.stacked) {
      for (let s2 = allSeries.length - 1; s2 >= 0; s2--) {
        ret.add(allSeries[s2]);
      }
    } else {
      for (let s2 = 0; s2 < allSeries.length; s2++) {
        ret.add(allSeries[s2]);
      }
    }
    return ret;
  }
  _initSerieVariables(series, i2, realIndex) {
    const w = this.w;
    const graphics = new Graphics(this.ctx);
    ...
    let longestSeries = series[i2].length === w.globals.dataPoints;
    this.elSeries.attr({
  ...
  _calculatePathsFrom({
    ...
    if (series[i2][0] === null) {
      for (let s2 = 0; s2 < series[i2].length; s2++) {
        if (series[i2][s2] !== null) {
          prevX = this.xDivision * s2;
          prevY = this.zeroY - series[i2][s2] / this.yRatio[translationsIndex];
  ...
  _handlePaths({ type, realIndex, i: i2, paths }) {
    const w = this.w;
    const graphics = new Graphics(this.ctx);
    const fill = new Fill(this.ctx);
    ...
    if (w.config.stroke.show && !this.pointsChart) {
      ...
      for (let p2 = 0; p2 < paths.linePaths.length; p2++) {
  ...
  _iterateOverDataPoints({
    ...
    for (let j = 0; j < iterations; j++) {
      if (series[i2].length === 0) break;
      const isNull = typeof series[i2][j + 1] === "undefined" || series[i2][j + 1] === null;
      if (w.globals.isXNumeric) {
      ...
      if (stackSeries) {
        if (i2 > 0 && w.globals.collapsedSeries.length < w.config.series.length - 1) {
          ...
          lineYPosition = this.prevSeriesY[prevIndex(i2 - 1)][j + 1];
        } else {
      ...
      if (isNull) {
        y2 = getY(minY, lineYPosition);
      } else {
        y2 = getY(series[i2][j + 1], lineYPosition);
        if (type === "rangeArea") {
          y22 = getY(seriesRangeEnd[i2][j + 1], lineYPosition);
        }
      }
      xArrj.push(series[i2][j + 1] === null ? null : x);
      ...
      this.handleNullDataPoints(series, pointsPos, i2, j, realIndex);
  ...
  _handleMarkersAndLabels({ type, pointsPos, isRangeStart, i: i2, j, realIndex }) {
    const w = this.w;
    let dataLabels = new DataLabels(this.ctx);
    if (!this.pointsChart) {
      if (w.globals.series[i2].length > 1) {
    ...
    switch (curve) {
      case "monotoneCubic": {
        ...
        let getSegments = (yArr, points) => {
          ...
          let segments = [];
          for (let i22 = 0, len = 0; i22 < segLens.length; len += segLens[i22++]) {
            segments[i22] = spline.slice(points, len, len + segLens[i22]);
          }
          return segments;
        };
        switch (pathState) {
          case 0:
            ...
            pathState = 1;
          case 1:
            if (!(rangeArea ? xArrj.length === series[i2].length : j === series[i2].length - 2)) {
              break;
            }
          case 2: {
            ...
            if (rangeArea && segmentCount > 1 && !isLowerRangeAreaPath) {
              ...
              upperLinePaths.forEach((u2) => linePaths.push(u2));
            }
            pathState = 0;
            break;
          }
        ...
        if (series[i2][j] === null) {
          ...
          switch (pathState) {
            case 0:
              ...
              if (series[i2][j + 1] === null || typeof series[i2][j + 1] === "undefined") {
                linePaths.push(linePath);
                areaPaths.push(areaPath);
                break;
              }
              pathState = 1;
              if (j < series[i2].length - 2) {
                let p2 = graphics.curve(pX + length, pY, x - length, y2, x, y2);
                linePath += p2;
                areaPath += p2;
                break;
              }
            case 1:
              if (series[i2][j + 1] === null) {
                if (isLowerRangeAreaPath) {
                  linePath += graphics.line(pX, y22);
                } else {
                  linePath += graphics.move(pX, pY);
                }
                ...
                if (j >= series[i2].length - 2) {
                  if (isLowerRangeAreaPath) {
                    linePath += graphics.curve(x, y2, x, y2, x, y22) + graphics.move(x, y22);
                  }
                  areaPath += graphics.curve(x, y2, x, y2, x, areaBottomY) + graphics.line(segmentStartX, areaBottomY) + "z";
                  linePaths.push(linePath);
                  areaPaths.push(areaPath);
                  pathState = -1;
                }
  ...
  handleNullDataPoints(series, pointsPos, i2, j, realIndex) {
    const w = this.w;
    if (series[i2][j] === null && w.config.markers.showNullDataPoints || series[i2].length === 1) {
...
var CircularChartsHelpers = class {
  ...
  drawYAxisTexts(x, y2, i2, text) {
    ...
    const yaxisLabel = graphics.drawText({
      ...
      y: y2 + yaxisConfig.labels.offsetY,
      text: formatter(text, i2),
      textAnchor: "middle",
      fontSize: yaxisConfig.labels.style.fontSize,
      fontFamily: yaxisConfig.labels.style.fontFamily,
      foreColor: Array.isArray(yaxisConfig.labels.style.colors) ? yaxisConfig.labels.style.colors[i2] : yaxisConfig.labels.style.colors
    });
    return yaxisLabel;
  }
};
var Pie = class {
  ...
  draw(series) {
    ...
    for (let i2 = 0; i2 < series.length; i2++) {
      let angle = this.fullAngle * Utils$1.negToZero(series[i2]) / total;
      sectorAngleArr.push(angle);
      if (this.chartType === "polarArea") {
        sectorAngleArr[i2] = this.fullAngle / series.length;
        this.sliceSizes.push(w.globals.radialSize * series[i2] / this.maxY);
      } else {
    ...
    if (w.globals.dataChanged) {
      ...
      for (let i2 = 0; i2 < w.globals.previousPaths.length; i2++) {
        previousAngle = this.fullAngle * Utils$1.negToZero(w.globals.previousPaths[i2]) / prevTotal;
  ...
  drawArcs(sectorAngleArr, series) {
    ...
    for (let i2 = 0; i2 < sectorAngleArr.length; i2++) {
      ...
      endAngle = startAngle + sectorAngleArr[i2];
      prevEndAngle = prevStartAngle + this.prevSectorAngleArr[i2];
      const angle = endAngle < startAngle ? this.fullAngle + endAngle - startAngle : endAngle - startAngle;
      let pathFill = fill.fillPath({
        seriesNumber: i2,
        size: this.sliceSizes[i2],
        value: series[i2]
      });
      let path = this.getChangedPath(prevStartAngle, prevEndAngle);
      let elPath = graphics.drawPath({
        d: path,
        stroke: Array.isArray(this.lineColorArr) ? this.lineColorArr[i2] : this.lineColorArr,
        strokeWidth: 0,
        fill: pathFill,
        fillOpacity: w.config.fill.opacity,
        classes: `apexcharts-pie-area apexcharts-${this.chartType.toLowerCase()}-slice-${i2}`
      });
      elPath.attr({
        index: 0,
        j: i2
      });
      filters.setSelectionFilter(elPath, 0, i2);
      if (w.config.chart.dropShadow.enabled) {
        const shadow = w.config.chart.dropShadow;
        filters.dropShadow(elPath, shadow, i2);
      }
      ...
      if (w.config.plotOptions.pie.expandOnClick && this.chartType !== "polarArea") {
        elPath.node.addEventListener("mouseup", this.pieClicked.bind(this, i2));
      }
      if (typeof w.globals.selectedDataPoints[0] !== "undefined" && w.globals.selectedDataPoints[0].indexOf(i2) > -1) {
        this.pieClicked(i2);
      }
      if (w.config.dataLabels.enabled) {
        ...
        if (angle !== 0 && w.config.plotOptions.pie.dataLabels.minAngleToShowLabel < sectorAngleArr[i2]) {
          let formatter = w.config.dataLabels.formatter;
          if (formatter !== void 0) {
          ...
          let foreColor = w.globals.dataLabels.style.colors[i2];
  ...
  pieClicked(i2) {
    ...
    let me = this;
    let path;
    let size = me.sliceSizes[i2] + (w.config.plotOptions.pie.expandOnClick ? 4 : 0);
    let elPath = w.globals.dom.Paper.findOne(
      `.apexcharts-${me.chartType.toLowerCase()}-slice-${i2}`
    );
    if (elPath.attr("data:pieClicked") === "true") {
      ...
      w.globals.capturedDataPointIndex = i2;
  ...
  renderInnerDataLabels(dataLabelsGroup, dataLabelsConfig, opts) {
    ...
    let x = opts.centerX;
    let y2 = !this.donutDataLabels.total.label ? opts.centerY - opts.centerY / 6 : opts.centerY;
    ...
    if (dataLabelsConfig.name.show) {
      let elLabel = graphics.drawText({
        x,
        y: y2 + parseFloat(dataLabelsConfig.name.offsetY),
        text: name2,
        textAnchor: "middle",
    ...
    for (let i2 = 0; i2 < w.globals.series.length; i2++) {
      spokes.push(
        Utils$1.polarToCartesian(
          ...
          w.config.plotOptions.pie.startAngle + angleDivision * i2
        )
      );
    }
    spokes.forEach((p2, i2) => {
      const line = graphics.drawLine(
        p2.x,
        p2.y,
        this.centerX,
        this.centerY,
        Array.isArray(spokeConfig.connectorColors) ? spokeConfig.connectorColors[i2] : spokeConfig.connectorColors
      );
      parent.add(line);
    });
  }
...
var Radar = class {
  ...
  draw(series) {
    ...
    series.forEach((s2, i2) => {
      let longestSeries = s2.length === w.globals.dataPoints;
      ...
      s2.forEach((dv, j) => {
        ...
        this.dataRadiusOfPercent[i2][j] = dv / range;
        this.dataRadius[i2][j] = this.dataRadiusOfPercent[i2][j] * this.size;
        this.angleArr[i2][j] = j * this.disAngle;
      });
      ...
      const defaultRenderedPathOptions = {
        ...
        bindEventsOnPaths: false,
        stroke: w.globals.stroke.colors[i2],
        strokeLineCap: w.config.stroke.lineCap
      };
      let pathFrom = null;
      if (w.globals.previousPaths.length > 0) {
      ...
      for (let p2 = 0; p2 < paths.linePathsTo.length; p2++) {
        let renderedLinePath = this.graphics.renderPaths(__spreadProps(__spreadValues({}, defaultRenderedPathOptions), {
          ...
          strokeWidth: Array.isArray(this.strokeWidth) ? this.strokeWidth[i2] : this.strokeWidth,
          fill: "none",
          drawShadow: false
        }));
      ...
      s2.forEach((sj, j) => {
        ...
        if (dataLabelsConfig.enabled) {
          let text = dataLabelsConfig.formatter(w.globals.series[i2][j], {
            seriesIndex: i2,
            dataPointIndex: j,
            w
          });
  ...
  drawPolygons(opts) {
    ...
    for (let i2 = 0; i2 < layers; i2++) {
      radiusSizes[i2] = layerDis * i2;
    }
    radiusSizes.reverse();
    let polygonStrings = [];
    let lines = [];
    radiusSizes.forEach((radiusSize, r2) => {
      ...
      polygon.forEach((p2, i2) => {
        if (r2 === 0) {
          const line = this.graphics.drawLine(
            ...
            0,
            0,
            Array.isArray(this.polygons.connectorColors) ? this.polygons.connectorColors[i2] : this.polygons.connectorColors
          );
          lines.push(line);
        }
        if (i2 === 0) {
    ...
    polygonStrings.forEach((p2, i2) => {
      ...
      const polygon = this.graphics.drawPolygon(
        p2,
        Array.isArray(strokeColors) ? strokeColors[i2] : strokeColors,
        Array.isArray(strokeWidth) ? strokeWidth[i2] : strokeWidth,
        w.globals.radarPolygons.fill.colors[i2]
      );
      parent.add(polygon);
    });
    lines.forEach((l2) => {
    ...
    if (w.config.yaxis[0].show) {
      this.yaxisLabelsTextsPos.forEach((p2, i2) => {
        const yText = helpers.drawYAxisTexts(p2.x, p2.y, i2, yaxisTexts[i2]);
  ...
  drawXAxisTexts() {
    ...
    w.globals.labels.forEach((label, i2) => {
      let formatter = w.config.xaxis.labels.formatter;
      let dataLabels = new DataLabels(this.ctx);
      if (polygonPos[i2]) {
        let textPos = this.getTextPos(polygonPos[i2], this.size);
        ...
        const dataLabelText = dataLabels.plotDataLabelsText({
          ...
          color: Array.isArray(xaxisLabelsConfig.style.colors) && xaxisLabelsConfig.style.colors[i2] ? xaxisLabelsConfig.style.colors[i2] : "#a8a8a8",
...
var Radial = class extends Pie {
  ...
  drawTracks(opts) {
    ...
    for (let i2 = 0; i2 < opts.series.length; i2++) {
      ...
      let pathFill = fill.fillPath({
        seriesNumber: 0,
        size: opts.size,
        fillColors: Array.isArray(trackConfig.background) ? trackConfig.background[i2] : trackConfig.background,
        solid: true
      });
      ...
      elPath.attr("id", "apexcharts-radialbarTrack-" + i2);
      this.animatePaths(elPath, {
  ...
  drawArcs(opts) {
    ...
    for (let i2 = reverseLoop ? opts.series.length - 1 : 0; reverseLoop ? i2 >= 0 : i2 < opts.series.length; reverseLoop ? i2-- : i2++) {
      ...
      this.ctx.series.addCollapsedClassToSeries(elRadialBarArc, i2);
      opts.size = opts.size - strokeWidth - this.margin;
      let pathFill = fill.fillPath({
        seriesNumber: i2,
        size: opts.size,
        value: opts.series[i2]
      });
      let startAngle = this.startAngle;
      let prevStartAngle;
      const dataValue = Utils$1.negToZero(opts.series[i2] > 100 ? 100 : opts.series[i2]) / 100;
      let endAngle = Math.round(this.totalAngle * dataValue) + this.startAngle;
      let prevEndAngle;
      if (w.globals.dataChanged) {
        prevStartAngle = this.startAngle;
        prevEndAngle = Math.round(
          this.totalAngle * Utils$1.negToZero(w.globals.previousPaths[i2]) / 100
        ) + prevStartAngle;
      }
      ...
      let elPath = graphics.drawPath({
        ...
        fill: "none",
        fillOpacity: w.config.fill.opacity,
        classes: "apexcharts-radialbar-area apexcharts-radialbar-slice-" + i2,
        strokeDashArray: dashArray
      });
      Graphics.setAttrs(elPath.node, {
        "data:angle": angle,
        "data:value": opts.series[i2]
      });
      if (w.config.chart.dropShadow.enabled) {
        const shadow = w.config.chart.dropShadow;
        filters.dropShadow(elPath, shadow, i2);
      ...
      if (this.barLabels.enabled) {
        ...
        let text = this.barLabels.formatter(w.globals.seriesNames[i2], {
          seriesIndex: i2,
          w
        });
        ...
        let textColor = this.barLabels.useSeriesColors ? w.globals.colors[i2] : w.config.chart.foreColor;
        if (!textColor) {
          textColor = w.config.chart.foreColor;
        }
        const x = barStartCords.x + this.barLabels.offsetX;
        const y2 = barStartCords.y + this.barLabels.offsetY;
        let elText = graphics.drawText({
          x,
          y: y2,
          text,
  ...
  onBarLabelClick(e2) {
    let seriesIndex = parseInt(e2.target.getAttribute("rel"), 10) - 1;
...
var RangeBar = class extends Bar {
  draw(series, seriesIndex) {
    ...
    for (let i2 = 0; i2 < series.length; i2++) {
      ...
      for (let j = 0; j < w.globals.dataPoints; j++) {
        const strokeWidth = this.barHelpers.getStrokeWidth(i2, j, realIndex);
        const y1 = this.seriesRangeStart[i2][j];
        const y22 = this.seriesRangeEnd[i2][j];
        let paths = null;
        let barXPosition = null;
        let barYPosition = null;
        const params = { x, y: y2, strokeWidth, elSeries };
        let seriesLen = this.seriesLen;
        if (w.config.plotOptions.bar.rangeBarGroupRows) {
          seriesLen = 1;
        }
        if (typeof w.config.series[i2].data[j] === "undefined") {
          break;
        }
        if (this.isHorizontal) {
          barYPosition = y2 + barHeight * this.visibleI;
          let srty = (yDivision - barHeight * seriesLen) / 2;
          if (w.config.series[i2].data[j].x) {
          ...
          if (w.globals.isXNumeric) {
            x = (w.globals.seriesX[i2][j] - w.globals.minX) / this.xRatio - barWidth / 2;
          }
          barXPosition = x + barWidth * this.visibleI;
          ...
          paths = this.drawRangeColumnPaths(__spreadValues({
            indexes: { i: i2, j, realIndex, translationsIndex },
            barWidth,
            barXPosition,
            zeroH,
            xDivision
  ...
  detectOverlappingBars({
    ...
    const w = this.w;
    let overlaps = [];
    let rangeName = w.config.series[i2].data[j].rangeName;
    const x = w.config.series[i2].data[j].x;
    const labelX = Array.isArray(x) ? x.join(" ") : x;
    const rowIndex = w.globals.labels.map((_) => Array.isArray(_) ? _.join(" ") : _).indexOf(labelX);
    const overlappedIndex = w.globals.seriesRange[i2].findIndex(
    ...
    if (this.isHorizontal) {
      ...
      if (overlappedIndex > -1 && !w.config.plotOptions.bar.rangeBarOverlap) {
        overlaps = w.globals.seriesRange[i2][overlappedIndex].overlaps;
  ...
  drawRangeColumnPaths({
    ...
    let w = this.w;
    const { i: i2, j, realIndex, translationsIndex } = indexes;
  ...
  drawRangeBarPaths({
    ...
    let x2 = this.preventBarOverflow(zeroW + y22 / this.invertedYRatio);
    const range = this.getRangeValue(realIndex, j);
  ...
  getRangeValue(i2, j) {
    const w = this.w;
    return {
      start: w.globals.seriesRangeStart[i2][j],
      end: w.globals.seriesRangeEnd[i2][j]
    };
  }
};
...
function emitCoordinates(coords, row, rowLen, rowSum, xoffset, yoffset, width, height) {
  if (width >= height) {
    ...
    let subY = yoffset;
    for (let i2 = 0; i2 < rowLen; i2++) {
      const h2 = row[i2] / areaWidth;
...
function generate(data, width, height) {
  ...
  for (let i2 = 0; i2 < n; i2++) {
    const rect = seriesRects[i2];
    const rx = rect[0];
    const ry = rect[1];
    const rw = rect[2] - rx;
    const rh = rect[3] - ry;
    results[i2] = squarify(
      normalize(data[i2], rw * rh),
      rx,
      ry,
      rw,
      rh
  ...
  draw(series) {
    ...
    nodes.forEach((node, i2) => {
      ...
      if (w.config.chart.dropShadow.enabled) {
        const shadow = w.config.chart.dropShadow;
        const filters = new Filters(this.ctx);
        filters.dropShadow(ret, shadow, i2);
      }
      ...
      node.forEach((r2, j) => {
        ...
        if (w.globals.dataChanged) {
          ...
          if (this.dynamicAnim.enabled && w.globals.shouldAnimate) {
            speed = this.dynamicAnim.speed;
            if (w.globals.previousPaths[i2] && w.globals.previousPaths[i2][j] && w.globals.previousPaths[i2][j].rect) {
              fromRect = w.globals.previousPaths[i2][j].rect;
            }
            this.animateTreemap(elRect, fromRect, toRect, speed);
          }
        }
        let fontSize = this.getFontSize(r2);
        let formattedText = w.config.dataLabels.formatter(this.labels[i2][j], {
          value: w.globals.series[i2][j],
          seriesIndex: i2,
          dataPointIndex: j,
          w
      ...
      if (w.config.series.length > 1 && seriesTitle && seriesTitle.show) {
        const sName = w.config.series[i2].name || "";
...
var UpdateHelpers = class {
      ...
      charts.forEach((ch, chartIndex) => {
        ...
        if (options2 && typeof options2 === "object") {
          ...
          if (overwriteInitialConfig) {
            ...
            if (options2.series) {
              for (let i2 = 0; i2 < w.globals.collapsedSeriesIndices.length; i2++) {
                let series = w.config.series[w.globals.collapsedSeriesIndices[i2]];
                w.globals.collapsedSeries[i2].data = w.globals.axisCharts ? series.data.slice() : series;
              }
              for (let i2 = 0; i2 < w.globals.ancillaryCollapsedSeriesIndices.length; i2++) {
                let series = w.config.series[w.globals.ancillaryCollapsedSeriesIndices[i2]];
                w.globals.ancillaryCollapsedSeries[i2].data = w.globals.axisCharts ? series.data.slice() : series;
              }
  ...
  _extendSeries(s2, i2) {
    ...
    return __spreadProps(__spreadValues({}, w.config.series[i2]), {
      name: s2.name ? s2.name : ser == null ? void 0 : ser.name,
      color: s2.color ? s2.color : ser == null ? void 0 : ser.color,
      type: s2.type ? s2.type : ser == null ? void 0 : ser.type,
      group: s2.group ? s2.group : ser == null ? void 0 : ser.group,
      hidden: typeof s2.hidden !== "undefined" ? s2.hidden : ser == null ? void 0 : ser.hidden,
...
var ApexCharts = class _ApexCharts {
  ...
  destroy() {
    ...
    if (chartID) {
      Apex._chartInstances.forEach((c2, i2) => {
  ...
  updateOptions(options2, redraw = false, animate = true, updateSyncedCharts = true, overwriteInitialConfig = true) {
    ...
    if (options2.series) {
      ...
      if (options2.series.length && options2.series[0].data) {
        options2.series = options2.series.map((s2, i2) => {
  ...
  appendData(newData, overwriteInitialSeries = true) {
    ...
    for (let i2 = 0; i2 < newSeries.length; i2++) {
      if (newData[i2] !== null && typeof newData[i2] !== "undefined") {
        for (let j = 0; j < newData[i2].data.length; j++) {
          newSeries[i2].data.push(newData[i2].data[j]);
        }
      }
    }
  ...
  static getChartByID(id) {
    ...
    const c2 = Apex._chartInstances.filter((ch) => ch.id === chartId)[0];
    return c2 && c2.chart;
  }
  /**
  ...
  static initOnLoad() {
    ...
    for (let i2 = 0; i2 < els.length; i2++) {
      const el = els[i2];
      const options2 = JSON.parse(els[i2].getAttribute("data-options"));
...
function u() {
  return u = Object.assign ? Object.assign.bind() : function(r2) {
    for (var e2 = 1; e2 < arguments.length; e2++) {
      var t2 = arguments[e2];
      for (var n in t2) ({}).hasOwnProperty.call(t2, n) && (r2[n] = t2[n]);
    }
    return r2;
...
function l(r2, e2) {
  var t2 = f({}, r2);
  return p(r2) && p(e2) && Object.keys(e2).forEach(function(n) {
    p(e2[n]) ? t2[n] = n in r2 ? l(r2[n], e2[n]) : e2[n] : t2[n] = e2[n];
  }), t2;
}
function y(r2, e2) {
  ...
  t2.add(r2), t2.add(e2);
  var n = Object.keys(r2), o2 = Object.keys(e2);
  if (n.length !== o2.length) return false;
...
function h(o2) {
  var i2 = o2.type, c2 = void 0 === i2 ? "line" : i2, a2 = o2.width, p2 = void 0 === a2 ? "100%" : a2, h2 = o2.height, d = void 0 === h2 ? "auto" : h2, v = o2.series, O = o2.options, g = o2.chartRef, m = function(r2, e2) {
    ...
    if (Object.getOwnPropertySymbols) {
      ...
      for (n = 0; n < i3.length; n++) t2 = i3[n], e2.includes(t2) || {}.propertyIsEnumerable.call(r2, t2) && (o3[t2] = r2[t2]);
    }
    return o3;
  }(o2, s), j = (0, import_react.useRef)(null), w = (0, import_react.useRef)(null), P = (0, import_react.useRef)(null), S = g || P, E = function() {
  ...
  return import_react.default.createElement("div", u({ ref: j }, D));
}
