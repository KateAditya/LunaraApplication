import React, { useState, useEffect } from 'react';
import { BiBlock, BiShow, BiLoaderAlt, BiUserCheck, BiMessageSquareDetail } from 'react-icons/bi';
import { UserDetails } from '../components/UserDetails';
import { usersApi } from '../api/users';
import type { User } from '../types/user';
import toast from 'react-hot-toast';

export const AutoblockedUsers: React.FC = () => {
    const [currentPage, setCurrentPage] = useState(1);
    const perPage = 10;

    // API Data state
    const [users, setUsers] = useState<User[]>([]);
    const [loading, setLoading] = useState(true);
    const [totalUsers, setTotalUsers] = useState(0);
    const [totalPages, setTotalPages] = useState(1);

    // Modal state
    const [showDetails, setShowDetails] = useState(false);
    const [selectedUser, setSelectedUser] = useState<User | null>(null);
    const [unblockingId, setUnblockingId] = useState<string | null>(null);

    const loadAutoblockedUsers = async () => {
        try {
            setLoading(true);
            const response = await usersApi.getAutoblockedUsers({
                page: currentPage,
                limit: perPage,
            });

            if (response.success) {
                const resData = response as any;
                if (resData.users) {
                    setUsers(resData.users);
                    setTotalUsers(resData.count || 0);
                    setTotalPages(resData.totalPages || 1);
                } else if (resData.data?.users) {
                    setUsers(resData.data.users);
                    setTotalUsers(resData.data.pagination?.total || 0);
                    setTotalPages(resData.data.pagination?.totalPages || 1);
                }
            } else {
                toast.error('Failed to load autoblocked users');
            }
        } catch (error) {
            console.error('Error loading autoblocked users:', error);
            toast.error('Failed to load autoblocked users');
        } finally {
            setLoading(false);
        }
    };

    useEffect(() => {
        loadAutoblockedUsers();
    }, [currentPage]);

    const handleView = (user: User) => {
        setSelectedUser(user);
        setShowDetails(true);
    };

    const handleUnblock = async (user: User) => {
        if (window.confirm(`Are you sure you want to unblock "${user.firstName} ${user.lastName}"? They will be allowed to log back in.`)) {
            try {
                setUnblockingId(user.id);
                const res = await usersApi.unblockUser(user.id);
                if (res.success) {
                    toast.success(`User "${user.firstName} ${user.lastName}" has been successfully unblocked!`);
                    loadAutoblockedUsers();
                } else {
                    toast.error(res.message || 'Failed to unblock user');
                }
            } catch (error) {
                console.error('Unblock error:', error);
                toast.error('Failed to unblock user');
            } finally {
                setUnblockingId(null);
            }
        }
    };

    return (
        <div>
            {/* Warning banner */}
            <div className="vz-card mb-3 animate-in animate-in-1" style={{ borderLeft: '4px solid #e6533c' }}>
                <div className="vz-card-body" style={{ padding: '1rem 1.25rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                        <div style={{
                            width: 38,
                            height: 38,
                            borderRadius: '50%',
                            background: 'rgba(230, 83, 60, 0.1)',
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'center',
                            color: '#e6533c',
                            fontSize: '1.25rem',
                            flexShrink: 0
                        }}>
                            <BiBlock />
                        </div>
                        <div>
                            <h6 style={{ margin: 0, fontWeight: 700, color: 'var(--vz-text-primary)' }}>Automatic Block Management</h6>
                            <p style={{ margin: 0, fontSize: '0.75rem', color: 'var(--vz-text-muted)', marginTop: '0.125rem' }}>
                                Users listed here have been automatically logged out and suspended from accessing Lunara due to receiving 10 or more blocks from other community members. You can review their cases and unblock them below.
                            </p>
                        </div>
                    </div>
                </div>
            </div>

            {/* Table */}
            <div className="vz-card animate-in animate-in-2">
                <div className="vz-card-body" style={{ padding: 0 }}>
                    <div className="vz-table-wrapper">
                        <table className="vz-table">
                            <thead>
                                <tr>
                                    <th style={{ width: 50 }}>#</th>
                                    <th>User</th>
                                    <th>Email / Phone</th>
                                    <th>Received Blocks</th>
                                    <th>Autoblocked Reason</th>
                                    <th>Suspension Date</th>
                                    <th style={{ width: 120 }}>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {loading ? (
                                    <tr>
                                        <td colSpan={7} style={{ textAlign: 'center', padding: '3rem', color: 'var(--vz-text-muted)' }}>
                                            <BiLoaderAlt className="vz-spin" style={{ fontSize: '1.5rem', marginBottom: '0.5rem', color: 'var(--vz-primary)' }} />
                                            <div>Loading autoblocked users...</div>
                                        </td>
                                    </tr>
                                ) : users.length === 0 ? (
                                    <tr>
                                        <td colSpan={7} style={{ textAlign: 'center', padding: '2rem', color: 'var(--vz-text-muted)' }}>
                                            No autoblocked users found. All clear!
                                        </td>
                                    </tr>
                                ) : (
                                    users.map((user: User, idx: number) => (
                                        <tr key={user.id || idx}>
                                            <td style={{ color: 'var(--vz-text-muted)' }}>
                                                {(currentPage - 1) * perPage + idx + 1}
                                            </td>
                                            <td>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
                                                    <div style={{
                                                        width: 32,
                                                        height: 32,
                                                        borderRadius: '50%',
                                                        background: `linear-gradient(135deg, #e6533c, #f59e0b)`,
                                                        display: 'flex',
                                                        alignItems: 'center',
                                                        justifyContent: 'center',
                                                        color: '#fff',
                                                        fontWeight: 600,
                                                        fontSize: '0.6875rem',
                                                        flexShrink: 0,
                                                    }}>
                                                        {user.firstName?.charAt(0)}{user.lastName?.charAt(0)}
                                                    </div>
                                                    <div>
                                                        <div style={{ fontWeight: 600, fontSize: '0.8125rem' }}>{user.firstName} {user.lastName}</div>
                                                        <div style={{ fontSize: '0.6875rem', color: 'var(--vz-text-muted)' }}>
                                                            Role: {user.role || 'User'}
                                                        </div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                <div style={{ fontSize: '0.8125rem', color: 'var(--vz-text-primary)' }}>{user.email}</div>
                                                <div style={{ fontSize: '0.6875rem', color: 'var(--vz-text-muted)' }}>{user.phone || '—'}</div>
                                            </td>
                                            <td>
                                                <span style={{
                                                    fontWeight: 700,
                                                    color: user.blockCount ? '#e6533c' : '#f59e0b',
                                                    fontSize: '0.875rem',
                                                    background: user.blockCount ? 'rgba(230, 83, 60, 0.1)' : 'rgba(245, 158, 11, 0.1)',
                                                    padding: '0.125rem 0.5rem',
                                                    borderRadius: '4px'
                                                }}>
                                                    {user.blockCount ? `${user.blockCount} blocks` : (user as any).noShowCount ? `${(user as any).noShowCount} no-shows` : '0 blocks'}
                                                </span>
                                            </td>
                                            <td>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.75rem', color: 'var(--vz-text-secondary)' }}>
                                                    <BiMessageSquareDetail style={{ flexShrink: 0 }} />
                                                    <span>{user.autoblockedReason || 'Received 10 blocks from other users.'}</span>
                                                </div>
                                            </td>
                                            <td style={{ color: 'var(--vz-text-muted)', fontSize: '0.75rem' }}>
                                                {user.updatedAt ? new Date(user.updatedAt).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' }) : 'N/A'}
                                            </td>
                                            <td>
                                                <div style={{ display: 'flex', gap: '0.375rem' }}>
                                                    <button
                                                        className="vz-btn-icon"
                                                        title="View Profile Details"
                                                        onClick={() => handleView(user)}
                                                    >
                                                        <BiShow />
                                                    </button>
                                                    <button
                                                        className="vz-btn vz-btn-success vz-btn-xs"
                                                        style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.6875rem', padding: '0.25rem 0.5rem' }}
                                                        onClick={() => handleUnblock(user)}
                                                        disabled={unblockingId === user.id}
                                                    >
                                                        {unblockingId === user.id ? (
                                                            <BiLoaderAlt className="vz-spin" />
                                                        ) : (
                                                            <BiUserCheck />
                                                        )}
                                                        <span>Unblock</span>
                                                    </button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
                        </table>
                    </div>
                </div>

                {/* Pagination */}
                {totalPages > 1 && (
                    <div style={{
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'space-between',
                        padding: '0.75rem 1.25rem',
                        borderTop: '1px solid var(--vz-border-color)',
                    }}>
                        <span style={{ fontSize: '0.75rem', color: 'var(--vz-text-muted)' }}>
                            Showing {(currentPage - 1) * perPage + 1}–{Math.min(currentPage * perPage, totalUsers)} of {totalUsers}
                        </span>
                        <div style={{ display: 'flex', gap: '0.25rem' }}>
                            <button
                                className="vz-btn vz-btn-outline vz-btn-sm"
                                disabled={currentPage === 1}
                                onClick={() => setCurrentPage(p => p - 1)}
                            >Prev</button>
                            {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => i + 1).map(p => (
                                <button
                                    key={p}
                                    className={`vz-btn vz-btn-sm ${p === currentPage ? 'vz-btn-primary' : 'vz-btn-outline'}`}
                                    onClick={() => setCurrentPage(p)}
                                >{p}</button>
                            ))}
                            <button
                                className="vz-btn vz-btn-outline vz-btn-sm"
                                disabled={currentPage === totalPages}
                                onClick={() => setCurrentPage(p => p + 1)}
                            >Next</button>
                        </div>
                    </div>
                )}
            </div>

            {/* Details Modal */}
            {showDetails && selectedUser && (
                <UserDetails
                    user={selectedUser}
                    onClose={() => { setShowDetails(false); setSelectedUser(null); }}
                />
            )}
        </div>
    );
};

export default AutoblockedUsers;
