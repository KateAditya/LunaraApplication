...
class _DiscoveryScreenState extends State<DiscoveryScreen> {
      ...
      if (mounted) {
        setState(() {
          ...
          if (rawPartyPlans.isNotEmpty) {
            combinedPosts.addAll(rawPartyPlans.map((plan) {
              final user = plan['user'] as Map<String, dynamic>? ?? {};
              final venue = plan['venue'] as Map<String, dynamic>? ?? {};
              
              String timeStr = plan['planDateTime'] ?? plan['createdAt'] ?? '';
              ...
              return {
                ...
                'city': user['city'] ?? venue['city'] ?? 'Unknown',
                'bio': user['bio'] ?? '',
                'gender': user['gender'] ?? 'Unknown',
                'venue': venue['name'] ?? 'Unknown',
                'content': plan['message'] ?? '',
                'time': timeStr,
                'coverImageUrl': venue['coverImageUrl'] ?? venue['imageUrl'] ?? '',
                'userId': user['id'],
                'user': user,
          ...
          if (rawStrangersMeet.isNotEmpty) {
            combinedPosts.addAll(rawStrangersMeet.map((meet) {
              final user = meet['user'] as Map<String, dynamic>? ?? {};
              final venue = meet['venue'] as Map<String, dynamic>? ?? {};
              
              String timeStr = meet['eventDateTime'] ?? meet['createdAt'] ?? '';
              if (timeStr.isNotEmpty) {
                try {
                  final dt = DateTime.parse(timeStr);
                  timeStr = DateFormat('MMM dd, hh:mm a').format(dt);
              ...
              return {
                ...
                'firstName': user['firstName'] ?? 'User',
                'lastName': user['lastName'] ?? '',
                'profilePhotoUrl': user['photoUrl'] ?? user['profilePhotoUrl'],
                ...
                'time': timeStr,
                'coverImageUrl': venue['imageUrl'] ?? venue['coverImageUrl'] ?? '',
                'userId': user['id'],
                'user': user,
          ...
          final fallbackCities = ['Pune', 'Mumbai', 'Delhi', 'Bengaluru', 'Goa'];
          ...
          if (_availableCities.isNotEmpty) {
            ...
            if (ApiService.selectedCity == null) {
               ...
               final match = _availableCities.where((c) => c.toLowerCase() == defaultCity.toLowerCase()).toList();
               ApiService.setSelectedCity(match.isNotEmpty ? match.first : _availableCities.first);
            }
          } else if (ApiService.selectedCity == null) {
             ApiService.setSelectedCity(_currentUser?.city ?? 'Mumbai');
          }
          _availableAreas = _allVenues
              .where((v) => ApiService.selectedCity == null || v.city.toLowerCase() == ApiService.selectedCity!.toLowerCase())
  ...
  List<Venue> get _filteredVenues {
    var liveVenues = _allVenues.where((v) => v.status?.toLowerCase() == 'live').toList();
    
    // Filter by selected city
    if (ApiService.selectedCity != null) {
      liveVenues = liveVenues.where((v) => v.city.toLowerCase() == ApiService.selectedCity!.toLowerCase()).toList();
    }

    // Filter by selected area
    if (_selectedArea != null) {
      liveVenues = liveVenues.where((v) => (v.area ?? v.addressLine1).trim().toLowerCase() == _selectedArea!.toLowerCase()).toList();
    }
  ...
  List<Map<String, dynamic>> get _filteredPartyPlans {
    ...
    if (ApiService.selectedCity != null) {
      plans = plans.where((p) => (p['city']?.toString() ?? '').toLowerCase() == ApiService.selectedCity!.toLowerCase()).toList();
    }
    return plans;
  }
  ...
  List<Map<String, dynamic>> get _filteredUsers {
    ...
    if (ApiService.selectedCity != null) {
      users = users.where((u) {
        final String? userCity = u['city'] ?? (u['profile'] is Map ? u['profile']['city'] : null);
        return (userCity?.toString() ?? '').toLowerCase() == ApiService.selectedCity!.toLowerCase();
      }).toList();
    }
    return users;
  }
  ...
  Widget build(BuildContext context) {
    ...
    const double searchBoxHeight = 70.0;
    final double fixedTopPadding = statusBarHeight + navbarHeight + searchBoxHeight;
                  ...
                  const Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        Text(
                          'UPCOMING NIGHTS',
                          style: TextStyle(fontFamily: 'AllroundGothic', letterSpacing: 2, fontWeight: FontWeight.bold, color: Colors.black, fontSize: 15),
                        ),
                      ],
                    ),
                  ),
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => AllVenuesScreen(venues: _allVenues))),
                        child: const Text(
                          'SEE ALL',
                          style: TextStyle(fontFamily: 'AllroundGothic', letterSpacing: 1, fontWeight: FontWeight.bold, color: LunaraTheme.electricViolet, fontSize: 15),
                        ),
                      ),
                    ],
                  ),
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      const Text(
                        'RECENT POSTS',
                        style: TextStyle(letterSpacing: 2, fontWeight: FontWeight.bold, color: Colors.black, fontSize: 15),
                      ),
                      GestureDetector(
                        onTap: () {
                          ...
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (_) => AllPostsScreen(posts: displayFeeds, venues: _allVenues),
                            ),
                          );
                        },
                        child: const Text(
                          'SEE ALL',
                          style: TextStyle(letterSpacing: 1, fontWeight: FontWeight.bold, color: LunaraTheme.electricViolet, fontSize: 15),
                        ),
                      ),
                    ],
                  ),
                          ...
                          child: GestureDetector(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Flexible(
                                  child: Text(
                                    (ApiService.selectedCity ?? _currentUser?.city ?? 'Locating...').toUpperCase(),
  ...
  Widget _buildSearchBox() {
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        hintStyle: const TextStyle(color: Colors.black38, fontSize: 14, fontWeight: FontWeight.w500),
            ...
            itemBuilder: (context, index, realIndex) {
              ...
              final imageUrl = ad['imagePath'] != null
                  ? (ad['imagePath'].toString().startsWith('http')
                      ...
                      : '${ApiService.baseUrl}${ad['imagePath']}')
                  : '';
              final venueName = ad['venue'] != null ? ad['venue']['name']?.toString() ?? '' : '';
              final venueArea = ad['venue'] != null ? ad['venue']['area']?.toString() ?? ad['venue']['addressLine1']?.toString() ?? '' : '';
              final venueCategory = ad['venue'] != null ? ad['venue']['category']?.toString() ?? '' : '';

              return GestureDetector(
                onTap: () {
                  Venue? matchingVenue;
                  try {
                    matchingVenue = _allVenues.firstWhere((v) => v.id == ad['venueId']);
                  } catch (_) {
                    matchingVenue = null;
                  }
                  if (matchingVenue != null) {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => VenueDetailScreen(venue: matchingVenue!.toMap()),
                      ),
                    );
                    ...
                    final Map<String, dynamic> minimalVenue = {
                      ...
                      'name': ad['venue']['name'] ?? '',
                      'city': ad['venue']['city'] ?? ApiService.selectedCity ?? '',
                      'area': ad['venue']['area'] ?? ad['venue']['addressLine1'] ?? '',
                          ...
                          children: [
                            if (venueCategory.isNotEmpty) ...[
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color: LunaraTheme.electricViolet.withValues(alpha: 0.85),
                  ...
                  Positioned(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [
                          BoxShadow(
                            color: const Color(0xFFE100FF).withValues(alpha: 0.35),
                            blurRadius: 8,
                    ...
                    child: Material(
                      ...
                      child: InkWell(
                        onTap: () {
                          ...
                          if (venueId != null) {
                            try {
                              matchingVenue = _allVenues.firstWhere((v) => v.id == venueId);
                            } catch (_) {}
                          }
                          ...
                          if (matchingVenue != null) {
                            ...
                            passVenueMap = Map<String, dynamic>.from(venueMap);
                          }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          ...
                          style: TextStyle(color: LunaraTheme.electricViolet, fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 2),
                        ),
                        ...
                        Row(
                          children: [
                            ...
                            const Icon(Icons.directions_run_rounded, color: Colors.grey, size: 14),
                            const SizedBox(width: 4),
                            Text(
                              distanceText,
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Container(height: 18, width: 160, decoration: BoxDecoration(color: Colors.grey[100], borderRadius: BorderRadius.circular(4))),
                      const SizedBox(height: 14),
                      Container(height: 12, width: 80, decoration: BoxDecoration(color: Colors.grey[100], borderRadius: BorderRadius.circular(4))),
                      const SizedBox(height: 18),
                      Container(height: 14, width: 120, decoration: BoxDecoration(color: Colors.grey[100], borderRadius: BorderRadius.circular(4))),
                    ],
                  ),
                ),
              ],
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final String? profilePhoto = (userObj['profilePhotoUrl'] ?? userObj['profilePhoto'] ?? feed['image'])?.toString();
          ...
          return RepaintBoundary(child: Container(
            ...
            decoration: BoxDecoration(
              gradient: coverImageUrl == null ? LunaraTheme.cardGradient : null,
              image: coverImageUrl != null ? DecorationImage(
            ...
            child: InkWell(
              onTap: () {
                final venue = _allVenues.firstWhere(
                  ...
                  orElse: () => _allVenues.isNotEmpty ? _allVenues.first : Venue(id: '0', name: venueName, city: 'Pune', addressLine1: 'Pune', averageRating: 0.0),
                );
                  ...
                  children: [
                    Row(
                      children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                            shape: BoxShape.circle,
                            border: Border.all(color: const Color(0xFF7F00FF).withValues(alpha: 0.15), width: 1.5),
                          ),
                          child: ClipOval(
                            child: (imageUrl != null && imageUrl.isNotEmpty)
                                ? Image.network(
                                    imageUrl,
                                    fit: BoxFit.cover,
                                    errorBuilder: (context, error, stackTrace) => Image.asset(LunaraTheme.defaultAvatar, fit: BoxFit.cover),
                                  )
                                : Image.asset(LunaraTheme.defaultAvatar, fit: BoxFit.cover),
                          ),
                        ),
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                (userObj['firstName'] != null && userObj['lastName'] != null)
                                    ? '${userObj['firstName']} ${userObj['lastName']}'
                                    : (userObj['userName'] ?? 'Lunara User'),
                                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: coverImageUrl != null ? Colors.white : Colors.black),
                                maxLines: 1,
                                overflow: TextOverflow.ellipsis,
                              ),
                              ...
                              Text(
                                feed['time'] ?? '',
                                style: TextStyle(color: coverImageUrl != null ? Colors.white70 : Colors.grey, fontSize: 9),
                              ),
                            ],
                          ),
                        ),
                    ...
                    Expanded(
                      child: Text(
                        ...
                        style: TextStyle(
                          fontSize: 12.5,
                          height: 1.4,
                          color: coverImageUrl != null ? Colors.white : Colors.black87,
                        ),
                        maxLines: 4,
                        overflow: TextOverflow.ellipsis,
                      ),
                    ...
                    Container(
                      ...
                      child: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          const Icon(Icons.star_rounded, color: Color(0xFF7F00FF), size: 12),
                          const SizedBox(width: 4),
                          Flexible(
                            child: Text(
                              venueName.toUpperCase(),
                              style: TextStyle(
                                color: coverImageUrl != null ? Colors.white : const Color(0xFF7F00FF),
                                fontSize: 9,
                                fontWeight: FontWeight.w900,
                                letterSpacing: 0.5,
                              ),
              ...
              itemBuilder: (context, index) {
                ...
                return GestureDetector(
                  onTap: () {
                    User? resolvedUser;
                    try {
                      resolvedUser = User.fromJson(Map<String, dynamic>.from(user));
                    } catch (_) {}
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                  ...
                  decoration: BoxDecoration(
                    gradient: LunaraTheme.cardGradient,
                    borderRadius: BorderRadius.circular(20),
                    border: Border.all(color: const Color(0xFF7F00FF).withValues(alpha: 0.08), width: 1.2),
                    boxShadow: [
                      BoxShadow(
                        color: const Color(0xFF7F00FF).withValues(alpha: 0.06),
                        blurRadius: 16,
                        ...
                        children: [
                          ...
                          GestureDetector(
                            onTap: () {
                              ...
                              final String? profilePhoto = (user['profilePhotoUrl'] ?? user['profilePhoto'] ?? user['image'])?.toString();
                                ...
                                MaterialPageRoute(
                                  builder: (_) => ChatScreen(
                                    user: {
                                      ...
                                      'image': avatarUrl,
                                      'isAsset': false,
                                      'online': user['isOnline'] == true || user['online'] == true,
                                    },
                                  ),
                                ),
                              );
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final isSelected = city.toLowerCase() == (ApiService.selectedCity ?? '').toLowerCase();
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        ...
                        padding: const EdgeInsets.all(16),
                        decoration: BoxDecoration(
                          color: isSelected ? LunaraTheme.electricViolet.withValues(alpha: 0.1) : Colors.grey[50],
                          borderRadius: BorderRadius.circular(16),
                          border: Border.all(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            if (isSelected)
                              const Icon(Icons.check_circle_rounded, color: LunaraTheme.electricViolet),
                          ],
                        ),
                      ),
                    );
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final isSelected = isAll ? _selectedArea == null : _selectedArea == area;
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(20),
                border: Border.all(
                  color: isSelected ? Colors.transparent : const Color(0x1A7F00FF),
                  width: 1.2,
                ),
