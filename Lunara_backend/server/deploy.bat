...
xcopy /E /I /Y "dist" "\\103.224.247.22\C$\inetpub\wwwroot\lunara-backend\dist"

REM Copy web.config to server
echo Copying web.config to server...
copy "web.config" "\\103.224.247.22\C$\inetpub\wwwroot\lunara-backend\"

REM Copy package.json for dependencies
echo Copying package.json to server...
copy "package.json" "\\103.224.247.22\C$\inetpub\wwwroot\lunara-backend\"

REM Copy .env file
echo Copying .env to server...
copy ".env" "\\103.224.247.22\C$\inetpub\wwwroot\lunara-backend\"
