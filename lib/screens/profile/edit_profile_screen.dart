...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  Future<void> _deletePhoto() async {
    ...
    if (photoId == null || photoId.isEmpty) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Cannot delete photo. Please refresh your profile.'),
  ...
  Future<void> _pickAndUploadPhoto({bool isMainProfilePhoto = true}) async {
    try {
      ...
      if (isMainProfilePhoto) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Please take a live selfie for verification.'),
          ),
        );
        ...
        if (selfieImage == null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Selfie is required for profile photo updates.'),
            ),
          );
          return;
        }
  ...
  void initState() {
    ...
    _minBudgetController = TextEditingController(
      text: u.minBudget?.toString() ?? '',
    );
    _maxBudgetController = TextEditingController(
      text: u.maxBudget?.toString() ?? '',
    );
    _prefGendersController = TextEditingController(
      text: u.preferredGenders.join(', '),
    );
    _minAgeController = TextEditingController(
      text: u.minAgePreference?.toString() ?? '',
    );
    _maxAgeController = TextEditingController(
      text: u.maxAgePreference?.toString() ?? '',
    );
    _matchDistanceController = TextEditingController(
      text: u.matchDistanceKm?.toString() ?? '10',
    );
  ...
  Widget _buildOtherPhotosGrid() {
    final otherPhotos = _localPhotoDetails.length > 1
        ? _localPhotoDetails.skip(1).toList()
        : <Map<String, String>>[];
          ...
          : Form(
              ...
              child: ListView(
                ...
                children: [
                  ...
                  _buildTextField(
                    'Music Preference (comma separated)',
                    _musicController,
                  ),
                  _buildTextField('Smoking Preference', _smokingController),
                  _buildTextField(
                    'Drink Preference (comma separated)',
                    _drinkController,
                  ),
                  const SizedBox(height: 16),
                  ...
                  _buildTextField(
                    'Preferred Genders (comma separated)',
                    _prefGendersController,
                  ),
                  Row(
                    children: [
                      Expanded(
