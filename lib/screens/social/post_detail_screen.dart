    ...
    if (age == null) {
      ...
      if (dobRaw != null) {
        ...
        if (dob != null) {
          ...
          age = today.year - dob.year - ((today.month < dob.month || (today.month == dob.month && today.day < dob.day)) ? 1 : 0);
        }
      }
    }
          ...
          children: [
            if (finalPhoto != null && finalPhoto.isNotEmpty && finalPhoto.startsWith('http'))
              Image.network(
                finalPhoto,
                fit: BoxFit.cover,
                errorBuilder: (context, error, stackTrace) => Container(decoration: const BoxDecoration(gradient: LunaraTheme.deepPurpleGradient)),
              )
